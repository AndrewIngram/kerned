...
for(const [name,type] of Object.entries({chromium,firefox,webkit})){
 ...
 await page.evaluate(()=>window.editorDiagnostics.select(1,0));await menu('Block quote');assert.equal(await page.locator('.quote-rule').count(),1);
 ...
 await page.evaluate(()=>window.editorDiagnostics.select(1,0));await menu('Bullet list');assert.equal(await page.locator('.list-marker').first().innerText(),'•');
 ...
 await page.evaluate(()=>window.editorDiagnostics.select(1,0));await page.keyboard.down('Shift');for(let i=0;i<5;i++)await page.keyboard.press('ArrowRight');await page.keyboard.up('Shift');
 ...
 await page.getByRole('button',{name:'Add comment',exact:true}).click();await page.getByRole('dialog').waitFor();await page.getByLabel('Reply',{exact:true}).fill('A comment');await settle();assert.equal((await page.evaluate(()=>window.editorDiagnostics.comments())).threads[0].messages[0].reply,'A comment');
