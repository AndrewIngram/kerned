...
for(const name of (process.env.BROWSERS??'chromium,firefox,webkit').split(',')){
  ...
  try{
    ...
    const done=()=>page.waitForFunction(()=>window.editorDiagnostics.probe([]).reflowPending===0,null,{timeout:60000});
    ...
    await page.locator('.text-capture').evaluate(el=>{
      ...
      const last=window.editorDiagnostics.read().nodes.at(-1);window.editorDiagnostics.select(last.id,last.text.length);
    });await settle();
    await page.locator('.text-capture').evaluate(el=>{
    ...
    assert.equal(await page.evaluate(()=>window.editorDiagnostics.read().nodes.length),original.length*2);
    assert.ok(await page.evaluate(()=>window.editorDiagnostics.probe([]).reflowPending)>1000,'Pasting defers offscreen paragraphs');
    ...
    assert.deepEqual(await page.evaluate(()=>window.editorDiagnostics.read().nodes),original,'Undo cancels pending insertion layout');
    ...
    await page.evaluate(id=>window.editorDiagnostics.select(id,0),last.id);await settle();await page.keyboard.type('Pasted edit ');await settle();
    assert.equal(await page.evaluate(id=>window.editorDiagnostics.probe([id]).nodes[0].text,last.id),'Pasted edit '+last.text,'Queued layout must not replace a subsequent edit');
