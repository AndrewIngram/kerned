...
function distribution(values){const a=[...values].sort((a,b)=>a-b);

return{median:a[Math.floor(a.length*.5)]??0,p95:a[Math.min(a.length-1,Math.floor(a.length*.95))]??0,max:a.at(-1)??0};}
...
for(const [name,type] of Object.entries({chromium,firefox,webkit})){
 ...
 try{for(const total of [2000,10000])for(let trial=0;trial<3;trial++){
  ...
  const m=await page.evaluate(()=>window.editorDiagnostics.metrics());assert.deepEqual(errors,[]);
...
try{for(const total of [2000,10000]){
 ...
 await page.goto(`${base}?stream=${total}&paused=1`);await page.waitForFunction(()=>window.editorDiagnostics);
 const cdp=await page.context().newCDPSession(page);await cdp.send('HeapProfiler.enable');

 async function heap(){await cdp.send('HeapProfiler.collectGarbage');await cdp.send('HeapProfiler.collectGarbage');

return await cdp.send('Runtime.getHeapUsage');}

 const baseline=await heap();await page.evaluate(()=>window.editorDiagnostics.resume());await page.waitForFunction(()=>window.editorDiagnostics.probe([]).complete,undefined,{timeout:120000});
 const loaded=await heap(),metrics=await page.evaluate(()=>window.editorDiagnostics.metrics());
 ...
 await writeFile('artifacts/editor-large-benchmark.json',JSON.stringify(report,null,2)+'\n');await page.close();
}}finally{await browser.close();}
