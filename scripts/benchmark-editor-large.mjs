    ...
    for (const total of [2000, 10000])
      for (let trial = 0; trial < 3; trial++) {
        ...
        const frames = m.frames
          .filter((f) => f.at >= m.firstCanvasFlushMs && f.at <= m.completedAt)
          .map((f) => f.gapMs);
        ...
        const row = {
          ...
          total,
          trial,
          firstCanvasFlushMs: m.firstCanvasFlushMs,
          firstLoaded: m.firstLoaded,
          completeFromNavigationMs: m.completedAt,
          loadAfterMountMs: m.completedAt - m.startedAt,
          batches: m.samples.length,
          chunkWorkMs: distribution(m.samples.map((s) => s.workMs)),
          chunkElapsedMs: distribution(m.samples.map((s) => s.elapsedMs)),
          frameGapMs: distribution(frames),
          framesOver32Ms: frames.filter((v) => v > 32).length,
          canvasDrawMs: distribution(m.paints),
          maxMounted: m.maxMounted,
          maxSubmitted: m.maxSubmitted,
          layoutCalls: m.layoutCalls,
          memory: m.memory,
        };

        assert.equal(row.firstLoaded, 32);
        assert.ok(row.maxMounted < 10 && row.maxSubmitted < 25);
        report.trials.push(row);
        await writeFile(
          'artifacts/editor-large-benchmark.json',
        ...
        console.log(
          ...
          total,
          trial,
          Math.round(row.firstCanvasFlushMs),
          Math.round(row.loadAfterMountMs),
          row.frameGapMs.max.toFixed(1),
        );
        await page.close();
      }
  } finally {
...
try {
  for (const total of [2000, 10000]) {
    ...
    await page.goto(`${base}?stream=${total}&paused=1`);
    ...
    const loaded = await heap(),
      metrics = await page.evaluate(() => window.editorDiagnostics.metrics());
