...
for(const [name,type] of Object.entries({chromium,firefox,webkit})){
 ...
 await page.evaluate(()=>window.hybridSpike.select(2,0));await settle();await label('Paragraph');
 ...
 await page.evaluate(()=>window.hybridSpike.select(1,0));await page.keyboard.press('Control+a');await settle();await menu('Block quote');assert.equal(await page.locator('[data-quote]').count(),1);await page.getByRole('button',{name:'Undo',exact:true}).click();await settle();assert.equal(await page.locator('[data-quote]').count(),0);
 await page.evaluate(()=>window.hybridSpike.select(1,0));await menu('Block quote');await page.keyboard.press('Control+a');await settle();await menu('Block quote');assert.equal(await page.locator('[data-quote]').count(),1);await menu('Block quote');assert.equal(await page.locator('[data-quote]').count(),0);
