...
try{
 ...
 const id=await page.evaluate(minimumId=>{const n=window.editorDiagnostics.read().nodes.find(n=>n.id>minimumId&&n.kind==='paragraph');window.editorDiagnostics.scrollTo(n.id,-30);

return n.id;},minimumId);

await settle();

 const point=await page.evaluate(id=>{const state=window.editorDiagnostics.read(),p=state.scene.find(p=>p.id===id),r=document.querySelector('canvas').getBoundingClientRect();

return{x:r.left+50,y:r.top+p.y-state.scroll+12};},id);
