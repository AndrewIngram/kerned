...
for(const [name,type] of Object.entries({chromium,firefox,webkit})){
 ...
 const id=await page.evaluate(minimumId=>{const n=window.hybridSpike.read().nodes.find(n=>n.id>minimumId&&n.kind==='paragraph');window.hybridSpike.scrollTo(n.id,-30);return n.id;},minimumId);await settle();
 const point=await page.evaluate(id=>{const state=window.hybridSpike.read(),p=state.scene.find(p=>p.id===id),r=document.querySelector('canvas').getBoundingClientRect();return{x:r.left+50,y:r.top+p.y-state.scroll+12};},id);
 await page.mouse.click(point.x,point.y);await settle();
 const before=await page.evaluate(id=>({scroll:scrollY,node:window.hybridSpike.read().nodes.find(n=>n.id===id),selection:window.hybridSpike.read().selection}),id);
 await page.keyboard.type('XYZ');await settle();
 const after=await page.evaluate(id=>({scroll:scrollY,node:window.hybridSpike.read().nodes.find(n=>n.id===id),selection:window.hybridSpike.read().selection,notice:document.querySelector('.input-notice')?.textContent}),id);
 ...
 assert.equal(await page.evaluate(id=>window.hybridSpike.read().nodes.find(n=>n.id===id).text,id),after.node.text);
 await page.getByRole('button',{name:'Undo',exact:true}).click();await settle();
 assert.equal(await page.evaluate(id=>window.hybridSpike.read().nodes.find(n=>n.id===id).text,id),before.node.text);
