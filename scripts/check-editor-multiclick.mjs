...
for(const [name,type] of Object.entries({chromium,firefox,webkit})){
 ...
 try{for(const width of [1100,390]){
  ...
  const point=async(id,x)=>{const s=await read(),p=s.scene.find(p=>p.id===id),b=await page.getByLabel('Canvas document').boundingBox();

return {x:b.x+28+x,y:b.y+p.y-s.scroll+12};};

  const selected=async()=>{const s=await read(),n=s.nodes.find(n=>n.id===s.selection.id);

return n.text.slice(Math.min(s.selection.anchor,s.selection.focus),Math.max(s.selection.anchor,s.selection.focus));};
  ...
  await page.goto('http://127.0.0.1:5173/extensions.html');await page.waitForFunction(()=>window.editorDiagnostics);await settle();
