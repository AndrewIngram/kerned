...
for (const name of (process.env.BROWSERS ?? 'chromium').split(',')) {
  ...
  try {
    for (let trial = 0; trial < Number(process.env.TRIALS ?? 1); trial++) {
      ...
      if (process.env.BASELINE_DIR) {
        for (const path of [
          ...
          const body = outputText.replace(
            /from (["'])(\.[^"']+)\1/g,
            (_, quote, specifier) => `from ${quote}${specifier}.ts${quote}`,
          );

          await page.route(`**/src/${path}*`, (route) =>
      ...
      await page.goto(
        process.env.FORMATTING_URL ?? 'http://127.0.0.1:5173/editor.html?sample=warbreaker',
      );
      await page.waitForFunction(() => window.editorDiagnostics?.probe([]).complete, undefined, {
        timeout: 120000,
      });
      await page.evaluate(() =>
        window.editorDiagnostics.select(window.editorDiagnostics.read().nodes[0].id, 0),
      );
      ...
      const result = await page.evaluate(async () => {
        ...
        const paragraphs = state.nodes.filter(
          (node) => node.kind === 'paragraph' && node.text.length,
        );
        ...
        return {
          ...
          characters: paragraphs.reduce((n, p) => n + p.text.length, 0),
          handlerMs,
          paintedMs,
          layouts: after.layoutCalls - before.layoutCalls,
          pending: api.probe([]).reflowPending,
          bold,
          selectionPreserved:
            JSON.stringify(state.selection) === JSON.stringify(original.selection),
        };
      });
      ...
      Object.assign(
        ...
        await page.evaluate(() => {
          ...
          return {
            completeMs: performance.now() - window.formattingStarted,
            maxFrameGapMs: Math.max(...window.formattingFrames.gaps),
      ...
      const undo = await page.evaluate(async () => {
        ...
        return {
          paintedMs,
          restored:
            JSON.stringify(window.editorDiagnostics.read().nodes) ===
            JSON.stringify(window.formattingOriginal.nodes),
        };
      });
      ...
      await writeFile(
        process.env.REPORT ?? 'artifacts/editor-formatting-benchmark.json',
        JSON.stringify(report, null, 2) + '\n',
      );
      await page.close();
      assert.ok(
        result.paintedMs < Number(process.env.MAX_PAINT_MS ?? 250),
        `Bold took ${result.paintedMs.toFixed(1)}ms; budget is ${process.env.MAX_PAINT_MS ?? 250}ms`,
      );
    }
  } finally {
    await browser.close();
