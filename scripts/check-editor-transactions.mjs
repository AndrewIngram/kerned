...
for(const [name,type] of Object.entries({chromium,firefox,webkit})){
 ...
 try{for(const width of [1100,420]){
  ...
  const saved=await page.evaluate(at=>window.editorDiagnostics.anchor(1,at,1),at);
  await page.evaluate(at=>window.editorDiagnostics.select(1,at),at);await settle();
  ...
  const resolved=await page.evaluate(a=>window.editorDiagnostics.resolveAnchor(a),JSON.parse(JSON.stringify(saved)));
  ...
  await page.evaluate(id=>window.editorDiagnostics.select(id,0),right);await settle();await page.keyboard.press('Backspace');await settle();
  ...
  await page.evaluate(()=>window.editorDiagnostics.resume());await page.waitForFunction(()=>window.editorDiagnostics.metrics().samples.length>3);
  await page.evaluate(()=>window.editorDiagnostics.pause());await settle();const loaded=(await read()).nodes.length;
  await page.getByRole('button',{name:'Undo',exact:true}).click();await settle();
  await page.getByRole('button',{name:'Undo',exact:true}).click();await settle();
  ...
  await page.evaluate(at=>window.editorDiagnostics.select(1,at),at);await settle();await page.keyboard.press('Enter');await settle();
  ...
  assert.equal((await page.evaluate(()=>window.editorDiagnostics.metrics())).stalePaints,0);assert.deepEqual(errors,[]);
