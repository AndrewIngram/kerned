...
for (const [name, type] of Object.entries({ chromium, firefox, webkit })) {
  ...
  try {
    ...
    await page.goto(process.env.EDITOR_URL ?? 'http://127.0.0.1:5173/extensions.html');
    ...
    const point = async (id, x) => {
      ...
      return {
        x: box.x + (28 + x) * state.zoom,
        y: box.y + (p.y + 15) * state.zoom - state.scroll,
      };
    };
    ...
    assert.equal(
      ...
      original[0].text.slice(0, a) + 'XYZ' + original[1].text.slice(h),
    );
    assert.ok(!(await read()).nodes.some((node) => node.id === 2));
    await page.getByRole('button', { name: 'Undo', exact: true }).click();
    await settle();
    assert.deepEqual((await read()).nodes, original);
    assert.equal((await read()).selection.anchorId, 1);
    assert.equal((await read()).selection.id, 2);
    await page.keyboard.press('Enter');
    await settle();
    state = await read();
    assert.equal(state.nodes[0].text, original[0].text.slice(0, a));
    assert.equal(state.nodes[1].text, original[1].text.slice(h));
    await page.getByRole('button', { name: 'Undo', exact: true }).click();
    await settle();
    assert.deepEqual((await read()).nodes, original);
    ...
    const reverseStart = state.selection.focus,
      reverseEnd = state.selection.anchor;

    await page.keyboard.press('Backspace');
    await settle();
    assert.equal(
      (await read()).nodes[0].text,
      original[0].text.slice(0, reverseStart) + original[1].text.slice(reverseEnd),
    );
    await page.getByRole('button', { name: 'Undo', exact: true }).click();
    await settle();
    assert.deepEqual((await read()).nodes, original);
    // Keyboard extension and shift-click preserve the anchor across paragraphs.
    const p1 = await point(1, 45);
    ...
    await drag(await point(2, 45), await point(4, 55));
    await settle();
    state = await read();
    assert.equal(state.selection.anchorId, 2);
    ...
    await settle();
    assert.ok(!(await read()).nodes.some((node) => node.id === 3));
    await page.getByRole('button', { name: 'Undo', exact: true }).click();
    await settle();
    assert.deepEqual((await read()).nodes, original);
    ...
    assert.equal(state.selection.id, 2);
    ...
    assert.equal(
      (await read()).nodes[0].text,
      original[0].text.slice(0, beforePaste.anchor) +
        'PASTE' +
        original[1].text.slice(beforePaste.focus),
    );
    await page.getByRole('button', { name: 'Undo', exact: true }).click();
    await settle();
    assert.deepEqual((await read()).nodes, original);
    console.log(
      name,
      'cross-paragraph drag, highlight, copy, replace, delete, Enter, keyboard, zoom and undo passed',
    );
