...
for(const [name,type] of Object.entries({chromium,firefox,webkit})){
 ...
 try{
  ...
  const before=await read(),at=before.selection.focus;
  ...
  for(const zoom of ['1','1.5']){
   ...
   const start={x:box.x+40*scale,y:box.y+42*scale},end={x:box.x+130*scale,y:start.y};
   await page.mouse.move(start.x,start.y);await page.mouse.down();await page.mouse.move(end.x,end.y,{steps:10});await page.mouse.up();await settle();
   const state=await read(),selection=state.selection;
   assert.notEqual(selection.anchor,selection.focus,`${name}: drag must select visible text`);
   assert.ok(await input.evaluate(el=>el===document.activeElement),`${name}: drag retains input focus`);
   await canvas.screenshot({path:`artifacts/pointer-${name}-${zoom}.png`});
   await page.keyboard.type('X');await settle();
  ...
  await page.mouse.click(rect.x+rect.width*.2,rect.y+rect.height/2);await settle();
  const left=(await read()).selection;
  assert.ok(await input.evaluate(el=>el===document.activeElement),`${name}: comment click retains text focus`);
  await page.mouse.click(rect.x+rect.width*.75,rect.y+rect.height/2);await settle();
  const right=(await read()).selection;
  assert.equal(right.id,2);assert.ok(right.focus>left.focus,`${name}: comment click uses exact text hit position`);
  ...
  await page.keyboard.type('X');await settle();
  assert.equal((await read()).nodes.find(n=>n.id===2).text,original.slice(0,right.focus)+'X'+original.slice(right.focus));
  await page.getByRole('button',{name:'Undo',exact:true}).click();await settle();
  await page.mouse.move(rect.x+rect.width*.2,rect.y+rect.height/2);await page.mouse.down();
  await page.mouse.move(rect.x+rect.width*.75,rect.y+rect.height/2,{steps:8});await page.mouse.up();await settle();
  const dragged=(await read()).selection;
  assert.ok(dragged.focus>dragged.anchor,`${name}: drag within comment selects text`);
  await page.keyboard.press('Escape');
  await highlight.focus();await page.keyboard.press('Enter');await settle();
  assert.ok(await page.getByRole('button',{name:'Close',exact:true}).evaluate(el=>el===document.activeElement),`${name}: keyboard activation still focuses comment panel`);
  assert.deepEqual(errors,[]);console.log(name,'click, type, drag, replace and undo passed');
