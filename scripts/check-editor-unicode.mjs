 ...
 if(name==='chromium'){
  const variants=await page.evaluate(async()=>{
   ...
   for(const storage of ['objects','packed','carets','shaping']){
    ...
    const layout=owned.engine.layout({id:1,text,spans:[{start:0,end:text.length,bold:true,italic:false}],width:400,size:24});
    layout.draw(surface.getCanvas(),0,0);surface.flush();
    results.push({storage,lines:layout.lines,caret:layout.geometry(text.length,text.length,false).caret});
    layout.dispose();owned.engine.clear();surface.delete();
   }
   return results;
  });
  for(const result of variants){assert.deepEqual(result.lines,variants[0].lines);assert.deepEqual(result.caret,variants[0].caret);assert.ok(result.caret.every(Number.isFinite));}
 }
 console.log(name,'emoji sequences, missing glyphs, accents, grapheme deletion, undo and safe unsupported input passed');
