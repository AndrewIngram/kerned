...
for (const [name, type] of Object.entries({ chromium, firefox, webkit })) {
  ...
  try {
    ...
    await page.goto(process.env.EDITOR_URL ?? 'http://127.0.0.1:5176/extensions.html');
    ...
    report.cases.push({ browser: name, version: browser.version(), checks, results });
    await writeFile(
      'artifacts/editor-container-benchmark.json',
      JSON.stringify(report, null, 2) + '\n',
    );
    console.log(
      name,
      checks,
      results.map(({ count, nested, medianMs, p95Ms }) => ({ count, nested, medianMs, p95Ms })),
    );
  } finally {
    await browser.close();
  }
