    ...
    for (const width of [1100, 390]) {
      ...
      const settle = async () => {
        await page.waitForFunction(
          () => document.querySelector('.find-count')?.getAttribute('aria-busy') !== 'true',
        );
        await page.evaluate(
          () =>
      ...
      const count = async () => {
        await page.locator('.find-count[aria-busy="false"]').waitFor();
      ...
      const pixels = () =>
        page.evaluate(() => {
          ...
          for (let i = 0; i < data.length; i += 4) {
            if (data[i] === 245 && data[i + 1] === 185 && data[i + 2] === 65) active++;
      ...
      assert.ok(
        await page.evaluate(() => {
          const bar = document.querySelector('.find-bar').getBoundingClientRect(),
            ...
            state = window.editorDiagnostics.read();

          return canvas.top + state.scene[0].y - state.scroll >= bar.bottom;
        }),
        'Floating bar leaves the first match visible',
      );
      let colors = await pixels();
      assert.ok(colors.active > 20 && colors.other > 20, 'Canvas draws current and other matches');
      ...
      assert.equal(await count(), 'No results');
      assert.equal(
        await page.getByRole('button', { name: 'Next match', exact: true }).isDisabled(),
        true,
      );
      await page.getByRole('button', { name: 'Match case', exact: true }).click();
      assert.equal(await count(), '1 of 2');
      ...
      assert.equal(await count(), 'No results');
      colors = await pixels();
      assert.equal(colors.active + colors.other, 0, 'No stale highlights');
      await input.fill('ideas');
      await settle();
      assert.deepEqual(
        await page.evaluate(() => window.editorDiagnostics.read().selection),
        selection,
      );
      assert.deepEqual(await page.evaluate(() => window.editorDiagnostics.history()), {
        undo: 0,
        redo: 0,
      });
      ...
      assert.equal(await input.inputValue(), 'ideas');
      await input.press('Escape');
      ...
      const cell = page.getByLabel('Cell 1, 1 text', { exact: true });
      await cell.fill('Table ideas, more ideas');
      await settle();
      ...
      await settle();
      assert.equal(
        await page.locator('[data-find-active="true"]').innerText(),
        'ideas',
        'Table cell current match is highlighted',
      );
      assert.deepEqual(
        await page.evaluate(() => window.editorDiagnostics.read().selection),
        tableSelection,
      ...
      await input.fill('Table ideas');
      await settle();
      assert.equal(await count(), '1 of 1');
      await page.getByRole('button', { name: 'Undo', exact: true }).click();
      await settle();
      assert.equal(await count(), 'No results', 'Undo refreshes open Find');
      await page.getByRole('button', { name: 'Redo', exact: true }).click();
      await settle();
      assert.equal(await count(), '1 of 1', 'Redo refreshes open Find');
      ...
      await page.waitForFunction(
        () => window.editorDiagnostics?.metrics().completedAt,
        {},
        { timeout: 60000 },
      );

      const before = await page.evaluate(() => ({
        selection: window.editorDiagnostics.read().selection,
        layouts: window.editorDiagnostics.metrics().layoutCalls,
      }));

      const apiTimings = await page.evaluate(async () => {
        const { createFind } = await import('/src/editor/index.ts'),
        ...
        return ['Breath', 'Vivenna', 'the', 'e', '[.*]'].map((query) => {
      ...
      await input.fill('Breath');
      await settle();
      const searchPaintMs = performance.now() - started;
      const total = await page.evaluate(() => window.editorDiagnostics.find().matches.length);
      assert.ok(total > 100);
      ...
      colors = await pixels();
      assert.ok(colors.active > 20, 'Far-offscreen final match is composed and visible');
      await page.screenshot({ path: `artifacts/editor-find-book-${name}-${width}.png` });
      await input.press('Enter');
      await settle();
      assert.deepEqual(
        await page.evaluate(() => window.editorDiagnostics.find().active),
        atFirst,
        'Book navigation wraps',
      );

      const after = await page.evaluate(() => ({
        selection: window.editorDiagnostics.read().selection,
        layouts: window.editorDiagnostics.metrics().layoutCalls,
        find: window.editorDiagnostics.find(),
      }));

      assert.deepEqual(after.selection, before.selection);
      assert.ok(
        after.layouts - before.layouts < 150,
        'Find composes nearby matches, not the whole book',
      );
      ...
      await page.waitForFunction(
        () => !window.editorDiagnostics.probe([]).reflowPending,
        {},
        { timeout: 30000 },
      );
      await input.press('Shift+Enter');
      await settle();
      assert.ok((await pixels()).active > 20);
      await input.press('Escape');
      await settle();
      assert.equal(await input.count(), 0);
