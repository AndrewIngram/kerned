...
return [r.left-v.left-(28+b.x)*s.zoom,r.top-v.top-(p.y+b.y)*s.zoom+s.scroll,r.width-b.width*s.zoom];});
  ...
  const block=page.locator('[data-widget="3"]');await block.getByRole('button').click();await page.waitForFunction(()=>window.editorDiagnostics.read().scene.find(p=>p.id===3).height>250);
  ...
  await page.evaluate(()=>window.editorDiagnostics.select(1,0));await settle();
  ...
  await page.evaluate(()=>window.editorDiagnostics.select(1,0));await settle();await page.keyboard.insertText('漢');await page.getByRole('status').filter({hasText:'Latin'}).waitFor();assert.ok(!(await read()).nodes[0].text.includes('漢'));
  await page.evaluate(i=>window.editorDiagnostics.select(1,i+1),atom.index);await settle();await page.keyboard.insertText('\u0301');await page.getByRole('status').filter({hasText:'Combining marks'}).waitFor();assert.equal((await read()).nodes[0].inline.length,1);
