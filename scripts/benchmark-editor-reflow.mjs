...
for(const [name,type] of Object.entries({chromium,firefox,webkit})){
 ...
 try{for(let trial=0;trial<3;trial++)for(const mode of trial%2?['eager','viewport']:['viewport','eager']){
  ...
  await page.evaluate(()=>{window.resizeFrames=[];window.resizeSampling=true;let last=performance.now();

function frame(now){window.resizeFrames.push(now-last);last=now;

if(window.resizeSampling)requestAnimationFrame(frame);}

requestAnimationFrame(frame);});
  await page.setViewportSize({width:700,height:950});
  await page.waitForFunction(g=>{const p=window.editorDiagnostics.probe([]);

return p.generation>g&&!p.reflowPending;},before.generation);
