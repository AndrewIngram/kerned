...
for(const [name,type] of Object.entries({chromium,firefox,webkit})){
 ...
 try{for(const width of [1100,390]){
  ...
  await page.getByRole('button',{name:'Italic',exact:true}).click();await settle();const styles=(await read()).nodes[0].marks;assert.ok(['bold','italic'].every(type=>styles.some(mark=>mark.mark.type===type)));
  await page.getByRole('button',{name:'Undo',exact:true}).click();await settle();assert.ok((await read()).nodes[0].marks.every(s=>!(s.mark.type==='italic')));
  await page.getByRole('button',{name:'Undo',exact:true}).click();await settle();assert.deepEqual((await read()).nodes,original);
  await page.getByRole('button',{name:'Redo',exact:true}).click();await settle();assert.ok((await read()).nodes[0].marks.some(s=>(s.mark.type==='bold')));
