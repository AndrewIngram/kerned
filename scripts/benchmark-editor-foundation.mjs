...
const report = {
  ...
  methodology: {
    input:
      'Browser beforeinput to second requestAnimationFrame, including frame scheduling, one key at a time',
    navigation: 'Browser keydown to second requestAnimationFrame for alternating PageDown/PageUp',
    memory:
      'Chromium JS heap and backing storage after forced GC, separate from timed interactions; excludes total process/GPU memory',
    loading: 'Existing streaming benchmark with find off',
    paste:
      'Existing synthetic rich-clipboard benchmark, including duplicate-content and undo/redo assertions',
  },
  trials: [],
};
...
async function existing(script, filename, environment) {
  execFileSync(process.execPath, [script], {
    env: { ...process.env, ...environment, REPORT: filename },
    stdio: 'inherit',
  });
...
for (let trial = 0; trial < trials; trial++) {
  const loading = await existing(
    'scripts/benchmark-editor-loading.mjs',
    `${directory}/loading-${trial}.json`,
    { BROWSERS: 'chromium', SAMPLES: 'warbreaker', FIND: 'off' },
  );

  const paste = await existing(
    'scripts/benchmark-editor-paste.mjs',
    `${directory}/paste-${trial}.json`,
    { BROWSERS: 'chromium' },
  );
  ...
  try {
    ...
    async function measuredKey(key, eventType) {
      await page.evaluate((eventTypeValue) => {
        ...
        document.addEventListener(
          ...
          () => {
            ...
            requestAnimationFrame(() =>
              requestAnimationFrame(() => {
                window.foundationFrame = performance.now() - started;
              }),
            );
          },
    ...
    assert.equal(
      ...
      'x'.repeat(12) + original,
    );
    const paging = [];

    for (let i = 0; i < 12; i++)
      paging.push(await measuredKey(i % 2 ? 'PageUp' : 'PageDown', 'keydown'));
    ...
    const row = {
      ...
      loading,
      paste,
      typingMs: distribution(typing),
      typingSamplesMs: typing,
      pagingMs: distribution(paging),
      pagingSamplesMs: paging,
      loadedHeap,
      afterInteractionHeap,
    };

    report.trials.push(row);
    await writeFile(`${directory}/baseline.json`, JSON.stringify(report, null, 2) + '\n');
    console.log(
      `Foundation trial ${trial + 1}/${trials}: typing ${row.typingMs.median.toFixed(1)} ms, paging ${row.pagingMs.median.toFixed(1)} ms`,
    );
  } finally {
    await browser.close();
  }
...
report.summary = Object.fromEntries(
  [
    ['firstUsableMs', (r) => r.loading.firstUsableMs],
    ['streamingMs', (r) => r.loading.loadMs],
    ['pasteHandlerMs', (r) => r.paste.handlerMs],
    ['pastePaintMs', (r) => r.paste.paintMs],
    ['typingFrameMs', (r) => r.typingMs.median],
    ['pagingFrameMs', (r) => r.pagingMs.median],
