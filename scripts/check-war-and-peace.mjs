...
assert.equal(digest(html), manifest.htmlSha256);
...
for (const [name, type] of Object.entries({ chromium, firefox, webkit })) {
  ...
  try {
    ...
    url.search = 'sample=war-and-peace&paused=1';
    ...
    await settle();
    assert.equal(
      await page.evaluate(() => window.editorDiagnostics.probe([3]).nodes[0].text),
      'Edited ' + first.text,
    );
    ...
    const fidelity = await page.evaluate((htmlValue) => {
      ...
      const mismatches = expected.flatMap((element, i) => {
        const node = nodes[i],
          level = element.tagName === 'H2' ? 2 : element.tagName === 'H3' ? 3 : undefined;

        return node &&
          normal(node.text) === normal(element.textContent) &&
          node.kind === (level ? 'heading' : 'paragraph') &&
          node.level === level
          ? []
          : [i];
      });
      ...
      const italic = nodes
        .flatMap((node) =>
          node.marks
            .filter((span) => span.mark.type === 'italic')
            .map((span) => node.text.slice(span.from, span.to)),
        )
        .join('');

      const expectedItalic = [...template.content.querySelectorAll('em')]
        .map((element) => element.textContent)
        .join('');

      const passages = [...template.content.querySelectorAll('.preformatted')].map((element) =>
        normal(element.textContent),
      );

      return {
        count: nodes.length,
        mismatches,
        italicVerified: compact(italic) === compact(expectedItalic),
        linesVerified: passages.every((text) =>
          nodes.find((node) => normal(node.text) === text)?.text.includes('\n'),
        ),
        words: nodes
          .map((node) => node.text)
          .join(' ')
          .split(/\s+/).length,
        last: nodes.at(-1).id,
        lastText: nodes.at(-1).text,
      };
    }, html);

    assert.equal(fidelity.count, manifest.blocks);
    assert.deepEqual(fidelity.mismatches, []);
    assert.equal(fidelity.words, manifest.words);
    assert.equal(fidelity.italicVerified, true);
    assert.equal(fidelity.linesVerified, true);
    await page.screenshot({ path: `artifacts/war-and-peace-${name}.png` });
    await page.getByRole('button', { name: 'Open document outline', exact: true }).focus();
    await page.locator('.outline-items button').last().click();
    await settle();
    assert.ok(await page.evaluate(() => scrollY > 10000));
    await page.keyboard.press('Escape');
    await page.evaluate((id) => window.editorDiagnostics.scrollTo(id), fidelity.last);
    await settle();
    await page.evaluate((id) => window.editorDiagnostics.select(id, 0), fidelity.last);
    await page.keyboard.type('Last page. ');
    await settle();
    assert.equal(
      await page.evaluate(
        (id) => window.editorDiagnostics.probe([id]).nodes[0].text,
        fidelity.last,
      ),
      'Last page. ' + fidelity.lastText,
    );
    await page.getByRole('button', { name: 'Undo', exact: true }).click();
    await settle();
    assert.equal(
    ...
    await page.waitForFunction(
      () =>
        ...
        document.querySelector('select[aria-label="Sample"]')?.disabled === false &&
        window.editorDiagnostics.probe([1]).nodes[0]?.text !== 'BOOK ONE: 1805',
    );
    ...
    await page.waitForFunction(() => window.editorDiagnostics.probe([]).count === 4);
    await page.goBack();
    await page.waitForFunction(
      () => window.editorDiagnostics.probe([1]).nodes[0]?.text === 'BOOK ONE: 1805',
    );
    ...
    assert.equal(
      requests.filter((urlValue) => urlValue.endsWith('/samples/war-and-peace.html')).length,
      1,
    );
    assert.equal(
      requests.filter((urlValue2) => urlValue2.endsWith('/samples/warbreaker.html')).length,
      1,
    );
    ...
    results.push({
      ...
      words: fidelity.words,
      headings: manifest.headings,
      italicVerified: true,
      initialLoadMs,
      elapsedIncludingPausedEditsMs: Math.round(metrics.completedAt - metrics.startedAt),
    ...
    url.pathname = '/extensions.html';
    url.search = 'sample=war-and-peace&paused=1';
    await editor.goto(url.href);
    await editor.waitForFunction(() => window.editorDiagnostics);
...
await writeFile(
  'artifacts/war-and-peace-checks.json',
  JSON.stringify({ htmlSha256: manifest.htmlSha256, results }, null, 2) + '\n',
);
