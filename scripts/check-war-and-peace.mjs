...
const base=process.env.HYBRID_URL??'http://127.0.0.1:5176/editor.html';
...
for(const [name,type] of Object.entries({chromium,firefox,webkit})){
  ...
  try{
    ...
    assert.equal(await page.evaluate(()=>window.hybridSpike.probe([3]).nodes[0].text),'Edited '+first.text);
    ...
    await page.evaluate(id=>window.hybridSpike.select(id,0),fidelity.last);await page.keyboard.type('Last page. ');await settle();
    assert.equal(await page.evaluate(id=>window.hybridSpike.probe([id]).nodes[0].text,fidelity.last),'Last page. '+fidelity.lastText);
    await page.getByRole('button',{name:'Undo',exact:true}).click();await settle();
    assert.equal(await page.evaluate(id=>window.hybridSpike.probe([id]).nodes[0].text,fidelity.last),fidelity.lastText);
    ...
    await page.waitForFunction(()=>new URL(location.href).searchParams.get('sample')==='warbreaker'&&document.querySelector('select[aria-label="Sample"]')?.disabled===false&&window.hybridSpike.probe([1]).nodes[0]?.text!=='BOOK ONE: 1805');
    await picker.selectOption('war-and-peace');
    await page.waitForFunction(()=>window.hybridSpike.probe([1]).nodes[0]?.text==='BOOK ONE: 1805');
    assert.deepEqual(await page.evaluate(()=>window.hybridSpike.probe([3]).nodes[0]),first);
    await picker.selectOption('minimal');await page.waitForFunction(()=>window.hybridSpike.probe([]).count===4);
    await page.goBack();await page.waitForFunction(()=>window.hybridSpike.probe([1]).nodes[0]?.text==='BOOK ONE: 1805');
    ...
    const hybrid=await browser.newPage();
    url.pathname='/hybrid-editor.html';url.search='sample=war-and-peace&paused=1';
    await hybrid.goto(url.href);await hybrid.waitForFunction(()=>window.hybridSpike);
    assert.equal(await hybrid.getByLabel('Sample').inputValue(),'war-and-peace');
    await hybrid.getByRole('heading',{name:'War and Peace',exact:true}).waitFor();
    assert.equal(await hybrid.evaluate(()=>window.hybridSpike.probe([]).count),32);
    await hybrid.close();
