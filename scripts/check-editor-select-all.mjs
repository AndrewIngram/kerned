...
for (const name of (process.env.BROWSERS ?? 'chromium,firefox,webkit').split(',')) {
  ...
  try {
    ...
    const result = async (label) => {
      ...
      return page.evaluate((labelValue) => {
        ...
        return {
          ...
          selection,
          selectedAll:
            selection.anchorId === state.nodes[0].id &&
            selection.anchor === 0 &&
            selection.id === last.id &&
            selection.focus === last.text.length,
        };
      }, label);
    };
    ...
    await page.mouse.click(canvas.x + 90, canvas.y + 48);
    cases.push(await result('clicked text'));
    ...
    assert.equal(
      native.id,
      4,
      'Native Select All must expand from the capture paragraph to the document',
    );
    await page.evaluate(() => window.editorDiagnostics.select(1, 0));
    await page.getByLabel('Sample', { exact: true }).selectOption('warbreaker');
    ...
    assert.ok(
      cases.every((row) => row.selectedAll),
      'Cmd-A must select the document from the page, canvas, and sample toolbar',
    );
  } finally {
    await browser.close();
  }
