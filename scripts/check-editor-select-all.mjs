  ...
  try{
    ...
    const result=async label=>{
      ...
      return page.evaluate(label=>{
        const state=window.hybridSpike.read(),last=state.nodes.at(-1),selection=state.selection;
    ...
    await page.evaluate(()=>window.hybridSpike.select(1,3));
    await page.evaluate(()=>new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve))));
    ...
    await page.evaluate(()=>new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve))));
    const native=await page.evaluate(()=>window.hybridSpike.read().selection);
    ...
    await page.getByLabel('Sample',{exact:true}).selectOption('warbreaker');await page.waitForFunction(()=>window.hybridSpike?.probe([]).complete);
    ...
    const beforeFind=await page.evaluate(()=>window.hybridSpike.read().selection);
    await find.press('Meta+a');
    assert.deepEqual(await find.evaluate(el=>[el.selectionStart,el.selectionEnd]),[0,6]);
    assert.deepEqual(await page.evaluate(()=>window.hybridSpike.read().selection),beforeFind);
    await find.press('Escape');await page.evaluate(()=>window.hybridSpike.select(1,0));
