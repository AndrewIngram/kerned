...
const artifact = 'artifacts/owned-blocks-large.json';
...
for (const [name, type] of Object.entries({ chromium, firefox, webkit })) {
 ...
 try {
  for (const count of [2000,10000]) for(const styled of [false,true]) {
   for(const api of styled ? ['blocks','whole'] : ['whole','blocks']) {
    const storage = 'shaping';
    ...
    try {
     ...
     await page.evaluate(({storage,count,styled,api})=>window.prepareOwnedLarge(storage,count,styled,api),{storage,count,styled,api});
     ...
     for(let trial=0;trial<3;trial++) {
      ...
      console.log(name,count,styled?'styled':'plain',api,trial,`cold ${cold.layoutMs.toFixed(1)}ms first ${streamed.firstSubmittedMs.toFixed(1)}ms total work ${streamed.totalWorkMs.toFixed(1)}ms`);
     }
     ...
     output.browsers[name].cases.push({count,styled,storage,api,trials,validation,memory:{baseline,loaded,released},errors});
