...
for(const [name,type] of Object.entries({chromium,firefox,webkit})){
 ...
 try{for(let trial=0;trial<3;trial++)for(const mode of trial%2?['eager','viewport']:['viewport','eager']){
  ...
  await page.goto(`${base}?stream=10000&reflow=${mode}`);await page.waitForFunction(()=>window.hybridSpike?.probe([]).complete&&!window.hybridSpike.probe([]).reflowPending);
  await page.evaluate(()=>window.hybridSpike.scrollTo(5011,8));await page.waitForTimeout(100);
  const before=await page.evaluate(()=>window.hybridSpike.probe([5011]));
  ...
  await page.waitForFunction(g=>{const p=window.hybridSpike.probe([]);return p.generation>g&&!p.reflowPending;},before.generation);
  await page.evaluate(()=>new Promise(r=>requestAnimationFrame(()=>{window.resizeSampling=false;r();})));
  const result=await page.evaluate(()=>({run:window.hybridSpike.metrics().reflows.at(-1),frames:window.resizeFrames,after:window.hybridSpike.probe([5011])}));
