...
for(const [name,type] of Object.entries({chromium,firefox,webkit})){
 ...
 try{
  ...
  for(let i=0;i<2;i++){
   ...
   await page.waitForFunction(()=>window.editorDiagnostics.read().nodes.length>4&&!window.editorDiagnostics.read().nodes[0].text.startsWith('Good ideas'));
  ...
  await page.goBack();await page.waitForFunction(()=>window.editorDiagnostics.read().nodes.length>4&&!window.editorDiagnostics.read().nodes[0].text.startsWith('Good ideas'));
  await page.goForward();await page.waitForFunction(()=>window.editorDiagnostics.read().nodes.length===4);
  await page.evaluate(()=>window.editorDiagnostics.select(1,0));await page.keyboard.type('Still editable. ');
  assert.ok(await page.evaluate(()=>window.editorDiagnostics.read().nodes[0].text.startsWith('Still editable. ')));
