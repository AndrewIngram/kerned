...
for(const [name,type] of Object.entries({chromium,firefox,webkit})){
 ...
 try{
  ...
  for(let i=0;i<2;i++){
   ...
   await page.waitForFunction(()=>window.hybridSpike.read().nodes.length>4&&!window.hybridSpike.read().nodes[0].text.startsWith('Good ideas'));
   ...
   await page.waitForFunction(()=>window.hybridSpike.read().nodes.length===4);
  }
  ...
  await page.goBack();await page.waitForFunction(()=>window.hybridSpike.read().nodes.length>4&&!window.hybridSpike.read().nodes[0].text.startsWith('Good ideas'));
  await page.goForward();await page.waitForFunction(()=>window.hybridSpike.read().nodes.length===4);
  await page.evaluate(()=>window.hybridSpike.select(1,0));await page.keyboard.type('Still editable. ');
  assert.ok(await page.evaluate(()=>window.hybridSpike.read().nodes[0].text.startsWith('Still editable. ')));
  assert.deepEqual(errors,[]);console.log(name,'in-place switching, cached book, back/forward and editing passed');
