...
for (const [name, type] of Object.entries({ chromium, firefox, webkit })) {
  ...
  try {
    for (const width of [1100, 390]) {
      ...
      await page.getByLabel('Cell 1, 1 text', { exact: true }).fill('Alpha');
      await settle();
      await page.keyboard.press('Tab');
      await page.getByLabel('Cell 1, 2 text', { exact: true }).fill('Beta');
      await settle();
      ...
      await settle();
      assert.equal(await page.locator('[data-cell]').count(), 12);
      await page.locator('.table-menu summary').click();
      await page.getByRole('button', { name: 'Add table column', exact: true }).click();
      await settle();
      assert.equal(await page.locator('[data-cell]').count(), 16);
      await page.getByRole('button', { name: 'Undo', exact: true }).click();
      await settle();
      assert.equal(await page.locator('[data-cell]').count(), 12);
      assert.deepEqual(errors, []);
      assert.ok(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth));
      await page.screenshot({ path: `artifacts/check-editor-tables-${name}-${width}.png` });
      console.log(
        name,
        'table insertion, cell editing, Tab, rectangular selection, deletion and undo passed',
      );
    }
  } finally {
    await browser.close();
