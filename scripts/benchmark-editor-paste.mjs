...
for (const name of (process.env.BROWSERS ?? 'chromium').split(',')) {
  ...
  try {
    ...
    const copy = await input.evaluate((el) => {
      ...
      window.pasteBenchmark = {
        data: Object.fromEntries([...data.types].map((type) => [type, data.getData(type)])),
      ...
      return {
        copyMs,
        characters: data.getData('text/plain').length,
        blocks: window.pasteBenchmark.nodes.length,
      };
    });
    ...
    const paste = await input.evaluate(async (el) => {
      ...
      for (const [type, value] of Object.entries(window.pasteBenchmark.data))
        event.clipboardData.setData(type, value);

      const before = window.editorDiagnostics.metrics().layoutCalls,
      ...
      return {
        ...
        layoutsToPaint: window.editorDiagnostics.metrics().layoutCalls - before,
        blocks: window.editorDiagnostics.read().nodes.length,
      };
    });
    ...
    await page.waitForFunction(() => !window.editorDiagnostics.probe([]).reflowPending, null, {
      timeout: 30000,
    });

    const background = await page.evaluate(() => ({
      completeMs: window.pasteBenchmark.completeMs,
      maxFrameGapMs: window.pasteBenchmark.maxFrameGapMs,
      stalePaints: window.editorDiagnostics.metrics().stalePaints,
    }));
    ...
    assert.equal(
      await page.evaluate(() => window.editorDiagnostics.read().nodes.length),
      copy.blocks * 2,
      'One redo restores the whole paste',
    );
    assert.deepEqual(errors, []);
    assert.equal(background.stalePaints, 0);
    const report = { browser: name, ...copy, originalBlocks: copy.blocks, ...paste, ...background };
    reports.push(report);
...
await writeFile(
  process.env.REPORT ?? 'artifacts/editor-paste-performance.json',
  JSON.stringify(reports, null, 2) + '\n',
);

for (const report of reports) {
  assert.ok(
    report.handlerMs < Number(process.env.MAX_HANDLER_MS ?? 200),
    'Pasting a book must not monopolize the input task',
  );
  assert.ok(
    report.paintMs < Number(process.env.MAX_PAINT_MS ?? 250),
    'Pasted text must reach a frame without laying out the whole book',
  );
}
