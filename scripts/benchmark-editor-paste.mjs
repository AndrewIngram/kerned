...
for(const name of (process.env.BROWSERS??'chromium').split(',')){
  ...
  try{
    ...
    const copy=await input.evaluate(el=>{
      ...
      window.pasteBenchmark={data:Object.fromEntries([...data.types].map(type=>[type,data.getData(type)])),html:data.getData('text/html'),nodes:window.hybridSpike.read().nodes};
    ...
    await page.evaluate(()=>{const last=window.hybridSpike.read().nodes.at(-1);window.hybridSpike.select(last.id,last.text.length);});await settle();
    const paste=await input.evaluate(async el=>{
      ...
      const before=window.hybridSpike.metrics().layoutCalls,started=performance.now();
      ...
      function frame(){
        ...
        if(window.hybridSpike.probe([]).reflowPending)requestAnimationFrame(frame);
      ...
      return {handlerMs,paintMs:performance.now()-started,layoutsToPaint:window.hybridSpike.metrics().layoutCalls-before,blocks:window.hybridSpike.read().nodes.length};
    });
    ...
    await page.waitForFunction(()=>!window.hybridSpike.probe([]).reflowPending,null,{timeout:30000});
    const background=await page.evaluate(()=>({completeMs:window.pasteBenchmark.completeMs,maxFrameGapMs:window.pasteBenchmark.maxFrameGapMs,stalePaints:window.hybridSpike.metrics().stalePaints}));
    ...
    assert.equal(await page.evaluate(()=>window.hybridSpike.read().nodes.length),copy.blocks,'One undo removes the whole paste');
    await page.getByRole('button',{name:'Redo',exact:true}).click();await settle();
    assert.equal(await page.evaluate(()=>window.hybridSpike.read().nodes.length),copy.blocks*2,'One redo restores the whole paste');
