...
for(const [name,type] of Object.entries({chromium,firefox,webkit})){
 ...
 const core=await page.evaluate(async()=>{
  ...
  const layout=id=>{
   ...
   return {lines,geometry:(a,h)=>({caret:[(h%10)*10,Math.floor(h/10)*20,0,Math.floor(h/10)*20+20]}),hit:(x,y)=>{const line=lines[Math.max(0,Math.min(lines.length-1,Math.floor(y/20)))];return {index:Math.max(line.start,Math.min(line.end,line.start+Math.round(x/10))),upstream:false};},move:(i,u,d)=>({index:d==='home'?Math.floor(i/10)*10:d==='end'?Math.min(b.text.length,(Math.floor(i/10)+1)*10):Math.max(0,Math.min(b.text.length,i+(d==='left'?-1:1))),upstream:false})};
  };
  const cases=[];
