...
try{
 ...
 await page.evaluate(()=>window.hybridSpike.select(1,0));
 await page.keyboard.press('ControlOrMeta+a');
 await page.locator('.text-capture').evaluate((el,text)=>{const event=new ClipboardEvent('paste',{bubbles:true,cancelable:true,clipboardData:new DataTransfer()});event.clipboardData.setData('text/plain',text);el.dispatchEvent(event);},text);
 assert.equal(await page.locator('.input-notice').innerText(),'');
 const expected=text.replace(/\t/g,' ').split('\n');
 const pasted=await page.evaluate(()=>window.hybridSpike.read().nodes.map(n=>n.text));
 assert.deepEqual(pasted,expected,'Every copied paragraph must survive paste');
 await page.getByRole('button',{name:'Undo',exact:true}).click();
 assert.equal(await page.evaluate(()=>window.hybridSpike.read().nodes.length),4);
 await page.getByRole('button',{name:'Redo',exact:true}).click();
 assert.deepEqual(await page.evaluate(()=>window.hybridSpike.read().nodes.map(n=>n.text)),expected);
 await page.getByRole('button',{name:'Undo',exact:true}).click();
 const original=await page.evaluate(()=>window.hybridSpike.read().nodes[0].text);
 for(const value of ['A\tB','one\r\ntwo\n\nthree\n']){
  await page.evaluate(()=>window.hybridSpike.select(1,5));
  await page.evaluate(()=>new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve))));
  await page.locator('.text-capture').evaluate((el,text)=>{const event=new ClipboardEvent('paste',{bubbles:true,cancelable:true,clipboardData:new DataTransfer()});event.clipboardData.setData('text/plain',text);el.dispatchEvent(event);},value);
  await page.evaluate(()=>new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve))));
  assert.equal(await page.locator('.input-notice').innerText(),'');
  const lines=value.replace(/\r\n?/g,'\n').replace(/\t/g,' ').split('\n');
  lines[0]=original.slice(0,5)+lines[0];lines[lines.length-1]+=original.slice(5);
  assert.deepEqual(await page.evaluate(count=>window.hybridSpike.read().nodes.slice(0,count).map(n=>n.text),lines.length),lines);
  await page.getByRole('button',{name:'Undo',exact:true}).click();
