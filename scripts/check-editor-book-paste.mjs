...
return event.clipboardData.getData('text/plain');});
 ...
 const pasted=await page.evaluate(()=>window.editorDiagnostics.read().nodes.map(n=>n.text));
 ...
 for(const value of ['A\tB','one\r\ntwo\n\nthree\n']){
  ...
  assert.deepEqual(await page.evaluate(count=>window.editorDiagnostics.read().nodes.slice(0,count).map(n=>n.text),lines.length),lines);
  await page.getByRole('button',{name:'Undo',exact:true}).click();
