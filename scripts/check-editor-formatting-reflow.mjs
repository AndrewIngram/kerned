...
for (const name of (process.env.BROWSERS ?? 'chromium,firefox,webkit').split(',')) {
  ...
  try {
    ...
    const click = async (nameValue) => {
      if (nameValue === 'Bold' || nameValue === 'Italic')
        await page.keyboard.press(nameValue === 'Bold' ? 'Meta+b' : 'Meta+i');
      else await page.getByRole('button', { name: nameValue, exact: true }).click();
      await settle();
    };
    ...
    await settle();
    await click('Bold');
    assert.ok((await pending()) > 1000, 'Formatting must defer offscreen layout');
    const firstPaint = await page.getByLabel('Canvas document').screenshot();
    await done();
    await settle();
    assert.ok(
      firstPaint.equals(await page.getByLabel('Canvas document').screenshot()),
      'Visible bold pixels must match completed layout',
    );
    const boldReference = await page.evaluate(() => window.editorDiagnostics.verifyReflow());
    await click('Undo');
    await done();
    assert.deepEqual(await page.evaluate(() => window.editorDiagnostics.read().nodes), original);
    // Undo while most pending paragraphs still contain their original layout.
    await click('Bold');
    assert.ok((await pending()) > 1000);
    await click('Undo');
    await done();
    assert.deepEqual(await page.evaluate(() => window.editorDiagnostics.read().nodes), original);
    const undoReference = await page.evaluate(() => window.editorDiagnostics.verifyReflow());
    ...
    assert.ok(
      ...
      'Jump must promote a paragraph before background completion',
    );
    ...
    assert.ok(
      ...
      `Background formatting must preserve scroll anchor: ${JSON.stringify({ before: { scroll: before.scroll, scene: before.scene }, after: { scroll: after.scroll, scene: after.scene } })}`,
    );
    assert.ok(
      jumpPaint.equals(await page.getByLabel('Canvas document').screenshot()),
      'Promoted viewport pixels must match final layout',
    );
    ...
    await writeFile(
      'artifacts/editor-formatting-reflow.json',
      JSON.stringify(results, null, 2) + '\n',
    );
    console.log(
      name,
      'bulk format, immediate undo/redo, superseding edits, resize, scroll anchors and eager geometry passed',
    );
  } finally {
    await browser.close();
  }
