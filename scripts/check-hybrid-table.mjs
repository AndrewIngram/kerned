...
for(const [name,type] of Object.entries({chromium,firefox,webkit})){
  ...
  try{
    ...
    const url=new URL(base);url.search='sample=warbreaker';
    await page.goto(url.href);await page.waitForFunction(()=>window.hybridSpike);
    const imported=await page.evaluate(()=>window.hybridSpike.importHtml('<p>Before</p><table><caption>Example</caption><tr><th colspan="2">Title</th></tr><tr><td rowspan="2"><strong>A</strong></td><td>B</td></tr><tr><td><u>C</u></td></tr></table><p>After</p>'));
    assert.deepEqual(imported.nodes.map(n=>n.kind),['paragraph','table','paragraph'],'HTML tables must remain a table block');
    ...
    await page.evaluate(id=>window.hybridSpike.scrollTo(id),bookTable.id);
    ...
    for(const [width,zoom] of [[1100,'1'],[420,'1.5']]){
      ...
      await page.waitForFunction(()=>window.hybridSpike.probe([]).reflowPending===0,null,{timeout:90000});
      await page.evaluate(id=>window.hybridSpike.scrollTo(id),bookTable.id);
      await page.evaluate(()=>new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r))));
      const geometry=await rendered.evaluate(el=>{const s=window.hybridSpike.read(),id=Number(el.dataset.table),p=s.scene.find(p=>p.id===id),next=s.scene[s.scene.findIndex(p=>p.id===id)+1],r=el.getBoundingClientRect();return {height:r.height/ s.zoom,expected:p.height,gap:next.y-p.y-p.height,overflow:document.documentElement.scrollWidth>innerWidth};});
