...
for(const [name,type] of Object.entries({chromium,firefox,webkit})){
 ...
 try{
  ...
  const result=await page.evaluate(async()=>{
   ...
   const nodes=[1,2].map(id=>({kind:'paragraph',id,key:`p${id}`,text:'Alpha beta gamma',marks:[{from:0,to:16,mark:{type:'bold',attrs:null}},{from:0,to:16,mark:{type:'italic',attrs:null}}],inline:[]}));
   ...
   const underline=editor.state.nodes.every(n=>n.marks.some(s=>(s.mark.type==='underline')));
   const unchanged=editor.state.selection.eq(selection);
   apply(textCommands(demoSchema,editor.state).clear());
   const cleared=editor.state.nodes[0].marks.every(s=>s.to<=6)&&editor.state.nodes[1].marks.every(s=>s.from>=10);
   editor.undo();const restored=textCommands(demoSchema,editor.state).active('underline');
   const {captureComment,createCommentStore,commentDecorations}=await import('/src/extensions/comment.ts');
   const {resolveDecorations}=await import('/src/editor/index.ts');
   const store=createCommentStore();store.put(captureComment(demoSchema,editor,'discussion',[]));
   const annotations=resolveDecorations(commentDecorations(store.state.threads),editor.positions).resolved[0].ranges;
