...
for(const [name,type] of Object.entries({chromium,firefox,webkit})){
 ...
 try{for(const [total,width,dpr] of [[2000,1100,1],[10000,1100,1],[10000,420,1.5]]){
  ...
  await page.goto(`${base}?stream=${total}&paused=1&slowImages=1`);await page.waitForFunction(()=>window.editorDiagnostics);await settle();
  assert.equal((await probe()).count,32);
  const initial=await page.evaluate(()=>window.editorDiagnostics.metrics());assert.equal(initial.firstLoaded,32);
  const input=page.getByLabel('Canvas text input');
  await page.evaluate(()=>window.editorDiagnostics.select(1,0));await settle();await page.keyboard.insertText('Edited ');await settle();
  ...
  await page.evaluate(()=>window.editorDiagnostics.pause());await settle();
  ...
  await page.evaluate(()=>window.editorDiagnostics.scrollTo(14,40));await settle();
  const beforeImage=await probe([14,15]);const imageShape=beforeImage.stats.shapeCalls;
  await page.locator('[data-image="14"] img').waitFor();await page.waitForFunction(()=>window.editorDiagnostics.probe([14]).scene[0].height>100);await settle();
  ...
  await page.evaluate(()=>window.editorDiagnostics.scrollTo(3));await settle();
  ...
  await block.getByLabel('Block notes').focus();await settle();await page.evaluate(()=>window.editorDiagnostics.scrollTo(4,8));await settle();
  const anchorBefore=await probe([4]);
  await block.getByLabel('Block notes').evaluate(el=>{el.style.height='210px';});
  await page.waitForFunction(()=>window.editorDiagnostics.probe([3]).scene[0].height>400);await settle();
  ...
  const atResume=await probe();await page.evaluate(()=>window.editorDiagnostics.resume());
  await page.waitForFunction(()=>window.editorDiagnostics.probe([]).complete,undefined,{timeout:120000});await settle();
  const discussions=await page.evaluate(()=>window.editorDiagnostics.comments());assert.equal(discussions.threads.length,1+Math.floor((total-4+10)/20));assert.equal(discussions.unresolved.length,0);assert.equal(discussions.resolved.length,discussions.threads.length);
  ...
  await page.evaluate(end=>window.editorDiagnostics.select(11,end),styledBefore.nodes[0].marks[0].to);await settle();await page.keyboard.insertText('\u0301');await settle();
  ...
  const resizeId=Math.floor(total/2)+6;await page.evaluate(id=>window.editorDiagnostics.scrollTo(id,8),resizeId);await settle();
  ...
  const metrics=await page.evaluate(()=>window.editorDiagnostics.metrics());assert.ok(metrics.maxMounted<10);assert.ok(metrics.maxSubmitted<25);assert.deepEqual(errors,[]);
