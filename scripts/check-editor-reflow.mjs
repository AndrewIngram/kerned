...
for(const [name,type] of Object.entries({chromium,firefox,webkit})){
 ...
 try{for(const total of [2000,10000]){
  ...
  const done=()=>page.waitForFunction(()=>window.editorDiagnostics.probe([]).reflowPending===0);
  await page.goto(`${url}?stream=${total}`);await page.waitForFunction(()=>window.editorDiagnostics?.probe([]).complete);await done();
  const middle=Math.floor(total/2)+7;await page.evaluate(id=>window.editorDiagnostics.scrollTo(id,8),middle);await settle();
  ...
  const reference=await page.evaluate(()=>window.editorDiagnostics.verifyReflow());assert.equal(reference.blocks,total);
  ...
  const reversal=await page.evaluate(()=>window.editorDiagnostics.metrics().reflows.at(-1));assert.ok(reversal.batches.reduce((n,b)=>n+b.layouts,0)<total/2,'Return to a cached width redid the entire document');
  await page.setViewportSize({width:520,height:950});await settle();
  const editId=total-9;await page.evaluate(id=>{window.editorDiagnostics.scrollTo(id);window.editorDiagnostics.select(id,0);},editId);await settle();
  ...
  await page.evaluate(()=>window.editorDiagnostics.scrollTo(3));await settle();const block=page.locator('[data-widget="3"]');await block.getByRole('button').click();await block.getByLabel('Block notes').fill('Keep focus during reflow.');await settle();
  ...
  await page.evaluate(()=>window.editorDiagnostics.scrollTo(4,8));await settle();const anchored=await probe([4]);await done();await settle();const final=await probe([4]);
  ...
  const finalReference=await page.evaluate(()=>window.editorDiagnostics.verifyReflow());assert.equal(finalReference.blocks,total);
