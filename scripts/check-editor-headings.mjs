...
for(const [name,type] of Object.entries({chromium,firefox,webkit})){
 ...
 await page.evaluate(()=>window.editorDiagnostics.select(1,5));await menu(`Heading ${level}`);
 ...
 await menu('Heading 2');await page.evaluate(()=>{const n=window.editorDiagnostics.read().nodes[0];window.editorDiagnostics.select(n.id,n.text.length);});await page.keyboard.press('Enter');await page.keyboard.type('Following paragraph');await settle();
 ...
 await page.evaluate(()=>window.editorDiagnostics.select(1,0));await page.keyboard.press('Control+a');await page.keyboard.type('Replacement');await settle();assert.equal((await read()).nodes.length,1);
 ...
 const page=await browser.newPage();await page.goto('http://127.0.0.1:5173/editor.html?sample=warbreaker');await page.waitForFunction(()=>window.editorDiagnostics?.read().nodes.some(n=>n.kind==='heading'));assert.ok((await page.evaluate(()=>window.editorDiagnostics.read().nodes)).some(n=>n.kind==='heading'));await page.close();
