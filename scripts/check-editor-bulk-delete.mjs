...
try{
  ...
  const results=await page.evaluate(async()=>{
    ...
    for(const inserted of ['', 'New text']){
      ...
      const anchors=[1,2,3,4,5,42,100].flatMap(id=>[0,2].flatMap(offset=>[-1,1].map(bias=>createAnchor(demoSchema,bulk.state,'doc',id,offset,bias))));
      ...
      apply(reference,{selection:command.selection,steps:[
    ...
    for(const step of [
      ...
      {ranges:[{kind:'text',id:2,from:0,to:1},{kind:'text',id:1,from:0,to:1}],pruneEmpty:[]},
    ...
    for(const count of [100,500])for(const mode of ['flat','nested','selection-api']){
      ...
      const planMs=performance.now()-started,planVisits=visits;
      visits=0;started=performance.now();
      const result=editor.dispatch({baseRevision:0,origin:'local',history:'separate',time:0,...command});
      const applyMs=performance.now()-started,applyVisits=visits;
      const after=JSON.stringify(editor.state.nodes);
      started=performance.now();editor.undo();const undoMs=performance.now()-started;
  ...
  for(const result of results){
    ...
    assert.ok(result.changes<10,'History must retain the replacement, not thousands of intermediate documents');
  }
