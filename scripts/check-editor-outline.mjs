...
for (const [name, type] of Object.entries({ chromium, firefox, webkit })) {
  ...
  try {
    ...
    await streaming.getByRole('button', { name: 'Open document outline', exact: true }).focus();
    assert.equal(
      await pendingButtons.count(),
      fullCount - 1,
      'Removed heading must not return from source outline',
    );
    ...
    console.log(
      name,
      'full pending outline, disabled navigation, live edits and progressive availability passed',
    );
    ...
    for (const width of [1100, 390]) {
      ...
      assert.deepEqual(
        ...
        before,
        'Navigation must preserve selection',
      );
      await page.keyboard.press('Escape');

      if (width > 600) {
        const marks = await page.locator('.outline-marks').evaluate((el) => {
          const r = el.getBoundingClientRect(),
            active = el.querySelector('[data-active="true"]').getBoundingClientRect(),
            all = el.querySelectorAll('.outline-mark');
          ...
          return {
            ...
            scroll: el.scrollTop,
            gap: all[1].getBoundingClientRect().top - all[0].getBoundingClientRect().top,
            overflow: getComputedStyle(el).overflow,
          };
        });

        assert.ok(marks.activeTop >= marks.top && marks.activeBottom <= marks.bottom);
        assert.equal(marks.gap, 10);
        assert.equal(marks.overflow, 'hidden');
        assert.ok(marks.scroll > 0);
      }
      ...
      await settle();
      assert.ok(await page.evaluate(() => scrollY < 30));
      await page.keyboard.press('Escape');
      assert.equal(await trigger.getAttribute('aria-expanded'), 'false');

      const rail = await page.locator('.document-outline').evaluate((el) => {
        const r = el.getBoundingClientRect(),
          toolbar = document.querySelector('.minimal-toolbar').getBoundingClientRect();

        return {
          center: r.top + r.height / 2,
          target: toolbar.height + (innerHeight - toolbar.height) / 2,
          height: r.height,
          max: (innerHeight - toolbar.height) * 0.8,
        };
      });

      assert.ok(Math.abs(rail.center - rail.target) < 1);
      assert.ok(rail.height <= rail.max + 1);
      await trigger.press('Enter');
      await page.locator('.outline-panel').waitFor();
      ...
      await trigger.focus();
      await page.locator('.outline-panel').waitFor();
      await page.screenshot({ path: `artifacts/outline-${name}-${width}.png` });
      await page.close();
      console.log(
        name,
        width,
        'outline navigation, dismissal, preserved selection and responsive fit passed',
      );
    }
    ...
    const result = await page.evaluate(async () => {
      ...
      const outline = createOutlineExtension(schema, (n) => {
        reads++;
      ...
      const a = outline.read(nodes),
        calls = reads,
        same = outline.read(nodes) === a && reads === calls;
    ...
    assert.equal(result.newReads, 1);
    assert.deepEqual(
      result.a.map((e) => [e.depth, e.parentKey]),
      [
        [0, null],
        [1, 'a'],
        [2, 'b'],
    ...
    assert.equal(result.edited[1].title, 'Changed');
    assert.equal(result.streamed.length, 2);
    await page.locator('.blocks-menu summary').click();
    ...
    await page.getByRole('button', { name: 'Open document outline', exact: true }).focus();
    await page.locator('.outline-panel').waitFor();
    assert.ok((await page.locator('.outline-items button').innerText()).startsWith('Renamed '));
    ...
    console.log(
      name,
      'generic extraction, nested/skipped levels, caching, streaming and editing passed',
    );
  } finally {
    await browser.close();
  }
