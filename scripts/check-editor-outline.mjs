...
for(const [name,type] of Object.entries({chromium,firefox,webkit})){
 const browser=await type.launch();try{
  ...
  for(const width of [1100,390]){
   ...
   const rail=await page.locator('.document-outline').evaluate(el=>{const r=el.getBoundingClientRect(),toolbar=document.querySelector('.minimal-toolbar').getBoundingClientRect();return {center:r.top+r.height/2,target:toolbar.height+(innerHeight-toolbar.height)/2,height:r.height,max:(innerHeight-toolbar.height)*.8};});assert.ok(Math.abs(rail.center-rail.target)<1);assert.ok(rail.height<=rail.max+1);
  ...
  const result=await page.evaluate(async()=>{
   ...
   let reads=0;const outline=createOutlineExtension(schema,n=>{reads++;return n.heading?{level:n.heading,title:n.label}:null;});
