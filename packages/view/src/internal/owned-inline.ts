...
import type { Glyph, Cluster, ParagraphGlyphs } from './owned-paragraph.js';
...
export function layoutInlineParagraph(
  text: string,
  spans: Span[],
  ...
  for (const atom of sorted) {
    if (
      ids.has(atom.id) ||
      indices.has(atom.index) ||
      !stops.has(atom.index) ||
      text[atom.index] !== '\ufffc' ||
      !stops.has(atom.index + 1)
    )
      throw new Error('Invalid inline atom position or identity');
    ...
    if (
      !(
        ...
        atom.ascent >= 0 &&
        atom.descent >= 0 &&
        atom.ascent + atom.descent > 0 &&
        [atom.width, atom.ascent, atom.descent].every(Number.isFinite)
      )
    )
      throw new Error('Invalid inline atom metrics');
    ids.add(atom.id);
  ...
  for (const span of spans)
    if (!(span.start < span.end && stops.has(span.start) && stops.has(span.end)))
      throw new Error('Invalid inline formatting');
  ...
  function segment(start: number, end: number) {
    ...
    const local = spans.flatMap((s) =>
      s.end > start && s.start < end
        ? [{ ...s, start: Math.max(start, s.start) - start, end: Math.min(end, s.end) - start }]
        : [],
    );
    ...
    if (local.length) {
      const cuts = [
        ...new Set([0, value.length, ...local.flatMap((s) => [s.start, s.end])]),
      ].toSorted((a, b) => a - b);
    ...
    for (const glyph of glyphs) {
      const shifted = { ...glyph, start: glyph.start + start };
      ...
      if (clusters.length === first || !cluster || cluster.start !== shifted.start) {
        ...
        cluster = { start: shifted.start, end, width: 0, glyphs: [], stops: [] };
        clusters.push(cluster);
      }

      cluster.glyphs.push(shifted);
      cluster.width += shifted.advance;
    }

    const points = boundaries(value);
    let cursor = 1;

    for (let c = first; c < clusters.length; c++)
      while (cursor < points.length && points[cursor] + start <= clusters[c].end) {
        clusters[c].stops.push(points[cursor++] + start);
      }
  }
  ...
  for (const atom of sorted) {
    segment(start, atom.index);
    breaks.add(atom.index);
    clusters.push({
      start: atom.index,
      end: atom.index + 1,
      width: atom.width,
