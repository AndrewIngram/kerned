// Adapted from bidi-js, commit 953d75cb176f0693aa77d60cdf3afde5f2a0927a.
// Copyright (c) 2021 Jason Johnston. MIT license: LICENSE-bidi-js.txt.
// Operates on Unicode scalar indices, never UTF-16 surrogate halves.
import {
  BN_LIKE_TYPES,
  getBidiCharType,
  ISOLATE_INIT_TYPES,
...
type Status = { level: number; override: number; isolate: number; isolInitIndex?: number };

type LevelRun = {
  start: number;
  end: number;
  level: number;
  startsWithPDI: boolean;
  endsWithIsolInit: number;
};

const nextEven = (n: number) => n + (n & 1 ? 1 : 2);

const nextOdd = (n: number) => n + (n & 1 ? 2 : 1);

export function getEmbeddingLevels(string: Uint32Array, baseDirection: 'ltr' | 'rtl' | 'auto') {
  const MAX_DEPTH = 125;
  ...
  // Match syntactic isolates once, including overflow-depth isolates. P2/FSI
  // lookahead must skip them without repeatedly scanning nested passages.
  const matchingPDI = new Int32Array(string.length).fill(-1);
  const openIsolates: number[] = [];

  for (let i = 0; i < string.length; i++) {
    const type = charTypes[i];

    if (type === TYPE_B) openIsolates.length = 0;
    else if (type & ISOLATE_INIT_TYPES) openIsolates.push(i);
    else if (type === TYPE_PDI) {
      const opener = openIsolates.pop();

      if (opener !== undefined) matchingPDI[opener] = i;
    }
  }

  const charTypeCounts = new Map<number, number>(); //will be cleared at start of each paragraph

  function changeCharType(i: number, type: number) {
  ...
  const paragraphs: Paragraph[] = []; // [{start, end, level}, ...]
  ...
  const FORMATTING_TYPES =
    TYPE_RLE | TYPE_LRE | TYPE_RLO | TYPE_LRO | ISOLATE_INIT_TYPES | TYPE_PDI | TYPE_PDF | TYPE_B;
  ...
  for (let paraIdx = 0; paraIdx < paragraphs.length; paraIdx++) {
    ...
    for (let runIdx = 0; runIdx < levelRuns.length; runIdx++) {
      ...
      if (!run.startsWithPDI || (run.startsWithPDI && !isolationPairs.has(run.start))) {
        const seqRuns: LevelRun[] = [(currentRun = run)];
        ...
        for (let i = 0; i < seqRuns.length; i++) {
          const sequenceRun = seqRuns[i];

          for (let j = sequenceRun.start; j <= sequenceRun.end; j++) {
            seqIndices.push(j);
          }
        }
        ...
        for (let i = seqIndices[0] - 1; i >= paragraph.start; i--) {
    ...
    for (let seqIdx = 0; seqIdx < isolatingRunSeqs.length; seqIdx++) {
      ...
      // W2. Retain the preceding strong type in one pass, including AL before
      // W3 changes it to R. Repeated backward scans make long digit runs quadratic.
      if (charTypeCounts.get(TYPE_EN)) {
        let precedingStrong = sosType;

        for (const index of seqIndices) {
          const type = charTypes[index];

          if (type & STRONG_TYPES) precedingStrong = type;
          else if (type === TYPE_EN && precedingStrong === TYPE_AL) changeCharType(index, TYPE_AN);
        }
      }
      ...
      if (charTypeCounts.get(TYPE_ES) || charTypeCounts.get(TYPE_CS)) {
        for (let si = 1; si < seqIndices.length - 1; si++) {
          ...
          if (charTypes[i] & (TYPE_ES | TYPE_CS)) {
            ...
            if (
              prevType === nextType &&
              (charTypes[i] === TYPE_ES ? prevType === TYPE_EN : prevType & (TYPE_EN | TYPE_AN))
            ) {
      ...
      if (
        ...
        for (let si = 0; si < seqIndices.length; si++) {
          ...
          if (charTypes[i] & (TYPE_ET | TYPE_ES | TYPE_CS)) {
            ...
            for (
              let sj = si + 1;
              sj < seqIndices.length && charTypes[seqIndices[sj]] & BN_LIKE_TYPES;
              sj++
            ) {
              changeCharType(seqIndices[sj], TYPE_ON);
            }
      ...
      if (charTypeCounts.get(NEUTRAL_ISOLATE_TYPES)) {
        ...
        const bracketPairs: [number, number][] = [];
        {
          const openerStack: { char: number; seqIndex: number }[] = [];
    ...
    for (let i = paragraph.start; i <= paragraph.end; i++) {
      ...
      if (type & BN_LIKE_TYPES) {
        embedLevels[i] = i === paragraph.start ? paragraph.level : embedLevels[i - 1];
      }
      ...
      if (i === paragraph.end || getBidiCharType(string[i]) & (TYPE_S | TYPE_B)) {
        for (let j = i; j >= paragraph.start && getBidiCharType(string[j]) & TRAILING_TYPES; j--) {
  ...
  function determineAutoEmbedLevel(start: number, isFSI: boolean) {
    // 3.3.1 P2 - P3
  ...
  function indexOfMatchingPDI(isolateStart: number) {
    return matchingPDI[isolateStart];
  }
}
