...
} from '../../../view/src/canvas/document-layout.js';
import { createOwnedEngine } from '../../../view/src/internal/owned-layout.js';
...
async function fixture(nodes: StarterNode[]) {
  ...
  const project = createDemoDocumentQuery(demoSchema);

  const controller = createDocumentLayout({
    owned,
    present: createFixturePresentation(20),
    source: {
