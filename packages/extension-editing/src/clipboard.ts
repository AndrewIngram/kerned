  ...
  fragments.set(token, {
    read<T extends NodeIdentity>(target: Schema<T>): ClipboardFragment<T> {
      if (Object.is(target, schema)) {
        const canonical: ClipboardFragment = fragment;
...
export function writeClipboard<N extends NodeIdentity>(
  ...
  state: EditorState<N>,
  text: string,
  serializer = createDocumentSerializer(schema, [...documentSerializers, ...tableSerializers], {
    unsupported: 'text',
  }),
) {
...
export function readClipboard<N extends NodeIdentity>(
  ...
  const nodes = (
    parser ?? createHtmlParser(schema, [...documentHtmlParsers, ...tableHtmlParsers])
  ).parse(source);
