...
export function replayRange(schema, editor, range) {
  ...
  for (const event of checkpoint.events)
    if (event.revision > range.start.revision)
      for (const op of event.operations) {
        const at = last.get(op.id),
          previous = at === undefined ? null : pending[at];

        if (previous && previous.inverse !== op.inverse) {
          pending[at] = null;
          last.delete(op.id);
        } else {
  ...
  function invert(map) {
    switch (map.kind) {
      ...
      default: {
        throw new Error('Unexpected mapping kind');
      }
    }
  }

  const maps = pending.flatMap((op) =>
    !op
      ? []
      : op.inverse
        ? [...definitions.get(op.id)].toReversed().map(invert)
        : definitions.get(op.id),
  );

  let start = { key: range.start.key, offset: range.start.offset },
    end = { key: range.end.key, offset: range.end.offset };
  ...
  function apply(point, map, bias, role) {
    ...
    if (
      map.kind === 'split' &&
      point.key === map.key &&
      (point.offset > map.at || (point.offset === map.at && bias === 1))
    )
    ...
    if (map.kind === 'replace' && point.key === map.key)
      return {
        key: point.key,
        offset:
          point.offset < map.from
            ? point.offset
            : point.offset > map.to
              ? point.offset + map.inserted - map.to + map.from
  ...
  for (const map of maps) {
    ...
    start = apply(start, map, range.start.association, 'start');
    end = apply(end, map, range.end.association, 'end');
  }

  if (!start || !end) return { status: 'deleted' };

  const texts = indexTree(schema, editor.state.nodes)
    .order.map((e) => e.node)
    .filter((n) => schema.text(n) !== null);

  const first = texts.findIndex((n) => n.key === start.key),
    lastIndex = texts.findIndex((n) => n.key === end.key);

  if (first < 0 || lastIndex < 0) return { status: 'deleted' };

  function snap(node, point, bias) {
    const stops = boundaries(schema.text(node));

    return bias === 1
      ? stops.find((at) => at >= point.offset)
      : stops.toReversed().find((at) => at <= point.offset);
  }

  start.offset = snap(texts[first], start, range.start.association);
  end.offset = snap(texts[lastIndex], end, range.end.association);

  if (
    first > lastIndex ||
    (first === lastIndex && (start.offset > end.offset || (!empty && start.offset === end.offset)))
  )
    return { status: 'deleted' };

  return {
    status: 'resolved',
    ranges: texts.slice(first, lastIndex + 1).map((node, i, nodes) => ({
      id: node.id,
      from: i === 0 ? start.offset : 0,
      to: i === nodes.length - 1 ? end.offset : schema.text(node).length,
    })),
  };
}
