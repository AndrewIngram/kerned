...
export function replayRange(schema, editor, range) {
  ...
  function snap(node, point, bias) {const stops = boundaries(schema.text(node)); return bias === 1 ? stops.find(at => at >= point.offset) : stops.reverse().find(at => at <= point.offset);}
