...
test('demo codecs preserve nested blocks, marks, mentions, locks and table structure', async () => {
  const result = await (async () => {
    const { demoDocumentCodec } = await import('../apps/demo/src/demo-schema.js');
    const { createSampleDocument } = await import('../apps/demo/src/sample-document.js');
    const { importHtml } = await import('../apps/demo/src/import-html.js');
