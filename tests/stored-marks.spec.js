...
test('caret toolbar formats subsequent typing and Enter without a document edit on toggle',async({page})=>{
 ...
 const before=await page.evaluate(()=>({revision:window.hybridSpike.comments().revision,history:window.hybridSpike.history()}));
 await expect(bold).toBeEnabled();await bold.click();await expect(bold).toHaveAttribute('aria-pressed','true');
 expect(await page.evaluate(()=>({revision:window.hybridSpike.comments().revision,history:window.hybridSpike.history()}))).toEqual(before);
 await page.keyboard.type('Bold');await page.keyboard.press('Enter');await page.keyboard.type('More');
 let nodes=await page.evaluate(()=>window.hybridSpike.read().nodes);
 expect(nodes[0].text).toBe('Bold');expect(nodes[0].spans.some(s=>s.bold&&s.start===0&&s.end===4)).toBe(true);
 expect(nodes[1].text.startsWith('More')).toBe(true);expect(nodes[1].spans.some(s=>s.bold&&s.start===0&&s.end===4)).toBe(true);
 await bold.click();await expect(bold).toHaveAttribute('aria-pressed','false');await page.keyboard.type('plain');
 nodes=await page.evaluate(()=>window.hybridSpike.read().nodes);expect(nodes[1].spans.filter(s=>s.bold).every(s=>s.end<=4)).toBe(true);
 ...
 await page.evaluate(()=>window.hybridSpike.select(1,0));await expect(bold).toHaveAttribute('aria-pressed','true');
 await page.evaluate(()=>window.hybridSpike.select(3,1));await expect(bold).toHaveAttribute('aria-pressed','false');
});
...
test('stored marks are explicit, reset on movement, restore with history and preserve graphemes',async({page})=>{
 const result=await page.evaluate(async()=>{
  ...
  const editor=createEditor(demoSchema,[{kind:'paragraph',id:1,key:'p',text:'abCD',spans:[{start:0,end:2,bold:true,italic:false}],atoms:[]}],textSelection(1,2));
  ...
  const plain=editor.state.nodes[0].spans.every(s=>s.end<=2);editor.undo();const undone=editor.state.storedMarks;editor.redo();const redone=editor.state.storedMarks;
  ...
  const grapheme=editor.state.nodes[0].spans.some(s=>s.bold&&s.start===0&&s.end>=2);
...
test('replacement inherits selected text and loading preserves a pending override',async({page})=>{
 const result=await page.evaluate(async()=>{
  ...
  const {demoSchema}=await import('/src/extensions/demo-schema.ts');
  const node=(id,text,spans=[])=>({kind:'paragraph',id,key:`p${id}`,text,spans,atoms:[]});
  ...
  const replacement=editor.state.nodes[0].spans.some(s=>s.bold&&s.start===1&&s.end===2);
...
test('caret formatting retains table input focus and formats cell text',async({page})=>{
 ...
 const cell=await page.evaluate(()=>window.hybridSpike.read().nodes.find(n=>n.kind==='table').rows[0][0].paragraphs[0]);
 expect(cell.text).toBe('Bold cell');expect(cell.spans.some(s=>s.bold&&s.start===0&&s.end===9)).toBe(true);
});
