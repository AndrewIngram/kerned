...
test('cell rectangles preserve formatting, grow tables, retain unaffected identities and undo atomically', async () => {
  const result = await (async () => {
    ...
    source.rows[2][1].paragraphs.push({
      kind: 'paragraph',
      ...allocate(),
    ...
    const selected = new tableCells.CellSelection(
      source.id,
      source.rows[2][2].id,
      source.rows[1][1].id,
    );
    ...
    const to = createEditor(
      demoSchema,
      [target],
      new tableCells.CellSelection(target.id, target.rows[1][1].id),
    ...
    const pasted = to.state.nodes[0],
      grid = tableCells.grid(selectionContext(demoSchema, to.state.nodes), pasted.id);

    const selectedCells = to.state.selection.cells(
      selectionContext(demoSchema, to.state.nodes),
    ).length;
    ...
    to.undo();
    const undone = JSON.stringify(to.state.nodes[0]) === JSON.stringify(original);
    to.redo();
    ...
    return {
      dimensions: [pasted.rows.length, pasted.rows[0].length],
      values: pasted.rows.map((row) => row.map((c) => c.paragraphs.map((p) => p.text))),
      heading: pasted.rows[1][1].paragraphs[0],
      sameCell: pasted.rows[1][1].id === original.rows[1][1].id,
      untouched: pasted.rows[0][0] === target.rows[0][0],
      selectedCells,
      fresh,
      undone,
      redo: JSON.stringify(to.state.nodes[0]) === JSON.stringify(pasted),
      plain,
  ...
  expect(result.dimensions).toEqual([3, 3]);
  expect(result.grid).toEqual([3, 3]);
  expect(result.selectedCells).toBe(4);
  expect(result.values[1].slice(1)).toEqual([['1,1'], ['1,2']]);
  expect(result.values[2].slice(1)).toEqual([['2,1', 'Second paragraph'], ['']]);
  expect(result.heading.kind).toBe('heading');
  expect(result.heading.marks[0].mark.type).toBe('bold');
  expect(
    result.sameCell &&
      result.untouched &&
      result.fresh &&
...
test('rectangle paste uses source size and rejects protected targets and merged grids without edits', async () => {
  const result = await (async () => {
    const {
      createEditor,
      createTable,
    ...
    editor.dispatch({
      ...
      history: 'separate',
      time: 0,
      ...pasteCellRectangle(demoSchema, editor.state, source, allocate),
    });
    const values = editor.state.nodes[0].rows.map((row) => row.map((c) => c.paragraphs[0].text));
    const blocked = table.rows[1][2].id;

    const secure = createEditor(
      demoSchema,
      [table],
      new tableCells.CellSelection(table.id, table.rows[1][1].id),
      [tableCells.extension],
      { permissions: { access: (n) => (n.id === blocked ? 'read-only' : 'editable') } },
    );
    ...
    try {
      secure.dispatch({
        ...
        history: 'separate',
        time: 0,
        ...pasteCellRectangle(demoSchema, secure.state, wide, allocate),
      });
    } catch {
      denied = true;
    }
    ...
    const whole = createEditor(
      demoSchema,
      [merged],
      new tableCells.CellSelection(merged.id, merged.rows[0][0].id, merged.rows[2][1].id),
      [tableCells.extension],
    );

    const copied = copyCellRectangle(demoSchema, whole.state);

    let rejected = false,
      partial = false;

    try {
      pasteCellRectangle(demoSchema, editor.state, copied, allocate);
    } catch {
      rejected = true;
    }

    whole.select(
      new tableCells.CellSelection(merged.id, merged.rows[0][1].id, merged.rows[2][1].id),
    );

    try {
      copyCellRectangle(demoSchema, whole.state);
    } catch {
      partial = true;
    }
    ...
    return {
      values,
      denied,
      unchanged: JSON.stringify(secure.state.nodes) === before && secure.state.revision === 0,
      rejected,
      partial,
      colspan: copied.rows[1][0].colspan,
    };
  })();
  ...
  expect(result.denied && result.unchanged && result.rejected && result.partial).toBe(true);
  expect(result.colspan).toBe(2);
});

test('large rectangles expand in both dimensions and retain header formatting', async () => {
  const result = await (async () => {
    const {
      createEditor,
      createTable,
    ...
    const editor = createEditor(
      demoSchema,
      [target],
      new tableCells.CellSelection(target.id, target.rows[20][20].id),
      [tableCells.extension],
    );

    const before = JSON.stringify(target),
      start = performance.now(),
      command = pasteCellRectangle(demoSchema, editor.state, source, allocate);

    editor.dispatch({ baseRevision: 0, origin: 'local', history: 'separate', time: 0, ...command });

    const elapsed = performance.now() - start,
      table = editor.state.nodes[0],
      grid = tableCells.grid(selectionContext(demoSchema, editor.state.nodes), table.id);

    const selected = editor.state.selection.cells(
      selectionContext(demoSchema, editor.state.nodes),
    ).length;
    ...
    return {
      ...
      last: table.rows[51][51].paragraphs[0].text,
      header: table.rows[20][20].header,
      selected,
      count,
      elapsed,
      undo: JSON.stringify(editor.state.nodes[0]) === before,
    };
  })();

  expect(result.dimensions).toEqual([52, 52]);
  expect(result.selected).toBe(1024);
  expect(result.last).toBe('31:31');
  expect(result.header && result.undo).toBe(true);
  expect(result.count).toBe(1 + 52 * 52 * 2);
