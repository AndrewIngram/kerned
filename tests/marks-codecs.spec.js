...
test('demo codecs preserve nested blocks, marks, mentions, locks and table structure',async({page})=>{
 const result=await page.evaluate(async()=>{
  ...
  const {createHybridDocument}=await import('/src/extensions/demo-model.ts');
  ...
  const sample=createHybridDocument(),sampleEncoded=demoDocumentCodec.encode(sample),sampleRound=demoDocumentCodec.encode(demoDocumentCodec.decode(JSON.parse(JSON.stringify(sampleEncoded))));
  const failures=[];
