...
test('table view routes copy, paste and cut through rich clipboard commands',async({page})=>{
 ...
 const copied=await page.evaluate(()=>{const event=new ClipboardEvent('copy',{bubbles:true,cancelable:true,clipboardData:new DataTransfer()});document.querySelector('[data-table]').dispatchEvent(event);return {html:event.clipboardData.getData('text/html'),text:event.clipboardData.getData('text/plain'),token:event.clipboardData.getData('application/x-gprose-fragment')};});
...
test('large rectangles expand in both dimensions and retain header formatting',async({page})=>{
 ...
 const result=await page.evaluate(async()=>{
  ...
  editor.undo();return {shape:[grid.width,grid.height],last:table.rows[51][51].paragraphs[0].text,header:table.rows[20][20].header,selected,count,elapsed,undo:JSON.stringify(editor.state.nodes[0])===before};
 });
