...
```tsx
function Card({ attributes, selection }: ReactNodeViewProps<typeof card>) {
  const theme = useContext(ApplicationTheme);
  return (
    <section className={theme.card} data-selected={selection.kind === 'node'}>
      {attributes.label}
    </section>
  );
}
...
available `width`, effective `access` and scoped `selection`. Attribute types come from the node's
...
the node, width, access and scoped selection are unchanged. Application context updates
