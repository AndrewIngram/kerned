...
export function sampleChunk(start: number, count: number): StarterNode[] {
  return Array.from({length: count}, (_, offset): StarterNode => {
    ...
    if (i % 20 === 0) return {kind:'checklist',id,key:`block-${id}`,checked:[false,false,false],expanded:false,notes:''};

    if (i % 20 === 4) return {kind:'image',id,key:`block-${id}`,src:imageSource,alt:`Landscape illustration for section ${index}`};
...
export type LoadSample = {loaded:number;count:number;workMs:number;elapsedMs:number;layouts:number};

export type ReflowSample={generation:number;width:number;blocks:number;started:number;firstPaintMs:number;completeMs:number;initialLayouts:number;batches:{workMs:number;layouts:number;background:number;pending:number}[]};

export function createStreamMetrics() {
  const reflows:ReflowSample[]=[];
  const widthChanges:{blocks:number;workMs:number}[]=[],editPaintMs:number[]=[];
  const samples:LoadSample[]=[],frames:{at:number;gapMs:number}[]=[],paints:number[]=[],lastLayoutIds:number[]=[];

  return {
    firstCanvasFlushMs:0, firstLoaded:0, startedAt:performance.now(), completedAt:0,
    samples, frames, widthChanges,editPaintMs,reflows,stalePaints:0,
    paints, maxMounted:0, maxSubmitted:0,
    layoutCalls:0, compositionMs:0, lastLayoutIds, lastSceneMs:0,
  };
}
