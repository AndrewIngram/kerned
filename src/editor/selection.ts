...
export function selectionContext<N extends NodeIdentity>(schema:Schema<N>,nodes:N[],tree:TreeIndex<N>=indexTree(schema,nodes)):SelectionContext{
  return {
    ...
    text(id){const node=tree.byId.get(id)?.node;

return node?schema.text(node):null;},
    selectable(id){const node=tree.byId.get(id)?.node;

return !!node&&schema.resolve(node).selectable!==false;},
    children(parent){if(parent===null)return nodes;const node=tree.byId.get(parent)?.node;

return node?schema.children(node):[];},
    location(id){const entry=tree.byId.get(id);
...
export function selectionMapping(before:SelectionContext,after:SelectionContext,maps:readonly PositionMap[]):SelectionMapping{
  return {
    ...
    point(point){let next={id:point.id,index:point.offset};
...
return {id:next.id,offset:next.index};},
    ...
    near(id){
      ...
      for(let i=at-1;i>=0;i--){const text=after.text(order[i].id);
...
function validPoint(context:SelectionContext,point:TextPoint){const text=context.text(point.id);

if(text===null)throw new Error('Selection requires a text endpoint');validateTextRange(text,point.offset,point.offset);}

export function selectionNear(context:SelectionContext,point?:TextPoint|null):Selection{
  if(point&&context.text(point.id)!==null){const text=context.text(point.id)??'';

if(point.offset>=0&&point.offset<=text.length){try{validPoint(context,point);

return new TextSelection(point,point);}catch{ /* A removed grapheme boundary needs a valid cursor. */ }}

return textSelection(point.id,0);}

  for(const node of context.order()){if(context.text(node.id)!==null)return textSelection(node.id,0);
...
if(text!==null)result.push({kind:'text',id:node.id,from:node.id===start.id?start.offset:0,to:node.id===end.id?end.offset:text.length});
  ...
  replace(context:SelectionContext,text:string):SelectionEdit{
    const ranges=this.ranges(context),first=ranges[0];
...
class TextBookmark implements SelectionBookmark{
  ...
  resolve(context:SelectionContext):Selection{const selection=new TextSelection(this.anchor,this.head,this.upstream);

try{selection.validate(context);

return selection;}catch{return selectionNear(context,this.head);}}
}
...
export class NodeSelection extends Selection{
  ...
  ranges(context:SelectionContext):SelectionRange[]{this.validate(context);

return [{kind:'node',id:this.id}];}
  getBookmark():SelectionBookmark{return new NodeBookmark(this.id);}
  encode(context:SelectionContext):SelectionJSON{this.validate(context);

return {type:this.type,version:1,data:{key:context.node(this.id)?.key}};}
  replace(context:SelectionContext,text:string):SelectionEdit{this.validate(context);

if(text)throw new Error('Replacing a node with text requires a schema insertion command');const location=context.location(this.id);
...
class NodeBookmark implements SelectionBookmark{
  ...
  map(mapping:SelectionMapping):SelectionBookmark{const id=mapping.node(this.id);

if(id!==null)return new NodeBookmark(id);const near=mapping.near(this.id);
...
export class AllSelection extends Selection{
  ...
  replace(context:SelectionContext,text:string):SelectionEdit{if(text)throw new Error('Replacing the document with text requires a schema insertion command');

return {steps:[{kind:'removeChildren',parent:null,index:0,count:context.children(null).length}],selection:new AllSelection()};}
}
...
function record(value:unknown):Record<string,unknown>{if(typeof value!=='object'||value===null||Array.isArray(value))throw new Error('Invalid selection data');

return Object.fromEntries(Object.entries(value));}

function writePoint(context:SelectionContext,point:TextPoint){validPoint(context,point);

return {key:context.node(point.id)?.key,offset:point.offset};}

function readPoint(context:SelectionContext,value:unknown):TextPoint{const data=record(value);

if(typeof data.key!=='string'||typeof data.offset!=='number'||!Number.isSafeInteger(data.offset))throw new Error('Invalid selection point');const node=context.byKey(data.key);

if(!node)throw new Error('Missing selection key');const point={id:node.id,offset:data.offset};validPoint(context,point);
...
export function createSelectionRegistry(extensions:readonly SelectionExtension[]=[]){
  ...
  const builtins:SelectionExtension[]=[
    ...
    {type:'text',read(context,value){const data=record(value);

if(typeof data.upstream!=='boolean')throw new Error('Invalid selection affinity');

return new TextSelection(readPoint(context,data.anchor),readPoint(context,data.head),data.upstream);}},
    {type:'node',read(context,value){const data=record(value);

if(typeof data.key!=='string')throw new Error('Invalid node key');const node=context.byKey(data.key);

if(!node)throw new Error('Missing node key');

return new NodeSelection(node.id);}},
    {type:'all',read(_context,value){if(value!==null)throw new Error('Invalid all selection');
...
return new AllSelection();}},
  ...
  return {
    ...
    read(context:SelectionContext,value:unknown){const data=record(value);

if(data.version!==1||typeof data.type!=='string')throw new Error('Invalid selection envelope');const extension=registered.get(data.type);

if(!extension)throw new Error('Unknown selection type');const selection=extension.read(context,data.data);

if(selection.type!==extension.type)throw new Error('Selection codec changed type');selection.validate(context);
