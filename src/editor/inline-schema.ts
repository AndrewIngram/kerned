...
export function createInlineSchema<Layout>(extensions:readonly InlineValueExtension<Layout>[]){
  ...
  function resolve(type:string){const extension=registry.get(type);if(!extension)throw new Error(`Unknown inline type: ${type}`);return extension;}
  ...
  return {
    ...
    decode(text:string,data:unknown):InlineValue[]{
      const values=jsonArray(data).map(raw=>{const value=jsonRecord(raw),type=jsonString(value.type);if(value.version!==resolve(type).version)throw new Error('Unsupported inline version');return create(type,jsonString(value.id),jsonNumber(value.index),value.attrs);});
      validateInlineObjects(text,values);return values;
    },
  };
}
