...
export function replaceRanges<N extends NodeIdentity>(schema:Schema<N>,nodes:N[],tree:TreeIndex<N>,ranges:readonly SelectionRange[],text:string,pruneEmpty:readonly number[],marks?:readonly Mark[]){
  ...
  const maps:PositionMap[]=[],anchorMaps:AnchorMap[]=[],joinedKeys=new Set<string>();
  ...
  for(const range of ranges){
    ...
    const node=tree.byId.get(range.id)?.node;if(!node)throw new Error('Missing selected block');
    if(range.kind==='node')continue;
    const editing=schema.editing(node),original=editing.text(node),inserted=range===first?text:'';
    ...
    if(next!==node){
      ...
      anchorMaps.push({kind:'replace',key:node.key,from:range.from,to:range.to,inserted:inserted.length});
    }
    if(!replacement){replacement=next;continue;}
    const left=replacement,at=schema.editing(left).text(left).length;
    replacement=schema.editing(left).join(left,next);
    if(replacement.id!==left.id||replacement.key!==left.key||schema.text(replacement)!==schema.editing(left).text(left)+schema.editing(next).text(next))throw new Error('Text extension violated join contract');
    ...
    anchorMaps.push({kind:'join',key:left.key,rightKey:node.key,at});joinedKeys.add(node.key);
  }
  ...
  for(const id of prunable){
    ...
    if(!node||schema.resolve(node).kind!=='container')throw new Error('Only containers can be pruned');
  }
  let visited=0;
  function rewrite(children:readonly N[]):N[]{
    ...
    for(const node of children){
      if(selected.has(node.id)){
        if(ranges[visited++]?.id!==node.id)throw new Error('Replacement ranges must follow document order without overlap');
        if(node.id===first.id)result.push(joined);
        continue;
      }
      const before=schema.children(node);
      if(!before.length){result.push(node);continue;}
      const after=rewrite(before);
      if(after.length===before.length&&after.every((child,i)=>child===before[i]))result.push(node);
      else if(after.length||!prunable.has(node.id))result.push(schema.withChildren(node,after));
    }
    return result;
  }
