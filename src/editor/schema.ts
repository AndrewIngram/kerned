...
export function createSchema<N extends NodeIdentity>(extensions:readonly NodeExtension<N>[]){
  ...
  return {
    ...
    withChildren(node:N,children:N[]):N{
      const extension=resolve(node);if(extension.kind!=='container')throw new Error('Target is not a container');
    ...
    validateChildren(node:N,parent:N|null){const extension=resolve(node);if(extension.kind==='container')extension.content.validateChildren(node,extension.content.children(node),{parent});},
    text(node:N){const extension=resolve(node);return extension.kind==='text'?extension.editing.text(node):null;},
    editing(node:N){const extension=resolve(node);if(extension.kind!=='text')throw new Error('This command requires editable text');return extension.editing;},
    validateUpdate(before:N,after:N){const extension=resolve(before);if(resolve(after)!==extension)throw new Error('Property updates cannot change node type');extension.validateUpdate(before,after);},
  };
}
