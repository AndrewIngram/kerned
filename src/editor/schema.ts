...
} & ({kind:'text';editing:TextBehavior<N>}|{kind:'atom'}|{kind:'container';content:{children(node:N):readonly N[];withChildren(node:N,children:N[]):N;validateChildren(node:N,children:readonly N[],context:{parent:N|null}):void}});
...
export function createSchema<N extends NodeIdentity>(extensions:readonly NodeExtension<N>[]){
  ...
  for(const extension of registered){if(!extension.name||!Number.isSafeInteger(extension.version)||extension.version<1)throw new Error('Extensions require a name and positive schema version');

if(names.has(extension.name))throw new Error(`Duplicate extension: ${extension.name}`);names.add(extension.name);}

  function resolve(node:N){
    const matches=registered.filter(extension=>extension.accepts(node));

    if(matches.length!==1)throw new Error(`Expected one extension for block ${node.key}, found ${matches.length}`);
  ...
  return {
    ...
    resolve,
    children(node:N):readonly N[]{const extension=resolve(node);
...
if(extension.kind!=='container')throw new Error('Target is not a container');
      ...
      if(next.id!==node.id||next.key!==node.key||resolve(next)!==extension)throw new Error('Container extension changed identity or type');
      const actual=extension.content.children(next);

      if(actual.length!==children.length||actual.some((child,i)=>child!==children[i]))throw new Error('Container extension changed child content');

      return next;
    },
    validateChildren(node:N,parent:N|null){const extension=resolve(node);

if(extension.kind==='container')extension.content.validateChildren(node,extension.content.children(node),{parent});},
    text(node:N){const extension=resolve(node);

return extension.kind==='text'?extension.editing.text(node):null;},
    editing(node:N){const extension=resolve(node);

if(extension.kind!=='text')throw new Error('This command requires editable text');

return extension.editing;},
    validateUpdate(before:N,after:N){const extension=resolve(before);

if(resolve(after)!==extension)throw new Error('Property updates cannot change node type');extension.validateUpdate(before,after);},
  };
}
