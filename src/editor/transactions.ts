...
export function applyTransaction<N extends NodeIdentity>(schema:Schema<N>,state:EditorState<N>,tx:Transaction<N>,selections=createSelectionRegistry(),permissions?:AccessPolicy<N>):Applied<N>{
  ...
  function treeFor(value:readonly N[]){let tree=indexes.get(value);

if(!tree){tree=indexTree(schema,value);indexes.set(value,tree);}
...
if(schema.text(node)===null)continue;
      ...
      else {const entry=byKey.get(node.key);
...
if(entry)entry.after=next;}
  ...
  function publish(next:N[],structural:boolean){
    ...
    if(structural){
      ...
      for(const [id,entry] of oldTree.byId){const nextEntry=newTree.byId.get(id);

if(nextEntry&&nextEntry.node.key!==entry.node.key)throw new Error('Structural changes cannot reuse an existing handle for a different key');}
...
for(const {node} of treeFor(step.nodes).order)changedIds.add(node.id);continue;
    }
...
while(count<journal.length&&journal[count].to<=through)count++;
    ...
    select(next:Selection){assertWritable();selections.validate(selectionContext(schema,state.nodes),next);const after={...state,selection:next,storedMarks:state.selection.eq(next)?state.storedMarks:null};prepareFields({kind:'selection',before:state,after});

if(!state.selection.eq(next))boundary=true;state=after;notify();
...
return state;},
    setStoredMarks(marks:readonly Mark[]|null){
      ...
      if(checked&&new Set(checked.map(mark=>mark.type)).size!==checked.length)throw new Error('Duplicate stored mark type');
      const after={...state,storedMarks:checked===null?null:structuredClone(checked)};prepareFields({kind:'storedMarks',before:state,after});state=after;boundary=true;notify();
...
return state;
    ...
    dispatch(tx:Transaction<N>){
      ...
      if(tx.origin==='local'&&result.changes.length){
        ...
        }else{past.push({changes:[...result.changes],maps:[...result.anchorMaps],positionMaps:[...result.maps],operations:[...operations],before:state.selection.getBookmark(),after:result.state.selection.getBookmark(),afterSelection:result.state.selection,beforeMarks:state.storedMarks??null,afterMarks:result.state.storedMarks??null,group,time:tx.time});
