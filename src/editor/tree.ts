...
export function indexTree<N extends NodeIdentity>(schema:Schema<N>,nodes:readonly N[]){
  const byId=new Map<number,TreeEntry<N>>(),byKey=new Map<string,TreeEntry<N>>(),order:TreeEntry<N>[]=[];
  const pending:TreeEntry<N>[]=nodes.map((node,index)=>({node,parent:null,index,path:[index]} satisfies TreeEntry<N>)).reverse();
...
export function validateTree<N extends NodeIdentity>(schema:Schema<N>,nodes:readonly N[],tree=indexTree(schema,nodes)){
  for(const {node,parent} of tree.order)schema.validateChildren(node,parent===null?null:tree.byId.get(parent)?.node??null);
  return tree;
}
export function childrenAt<N extends NodeIdentity>(schema:Schema<N>,nodes:readonly N[],parent:number|null,tree=indexTree(schema,nodes)):readonly N[]{
...
export function spliceChildren<N extends NodeIdentity>(schema:Schema<N>,nodes:N[],parent:number|null,index:number,count:number,inserted:readonly N[],tree=indexTree(schema,nodes)):N[]{
  const entry=parent===null?undefined:tree.byId.get(parent);
  if(parent!==null&&!entry)throw new Error('Missing container');
  if(entry&&schema.resolve(entry.node).kind!=='container')throw new Error('Target is not a container');
  const children=entry?schema.children(entry.node):nodes;
  if(!Number.isSafeInteger(index)||!Number.isSafeInteger(count)||index<0||count<0||index+count>children.length)throw new Error('Invalid child range');
  ...
  for(let depth=entry.path.length-1;depth>0;depth--){
    ...
    const siblings=[...schema.children(ancestor)];siblings[entry.path[depth]]=replacement;
    replacement=schema.withChildren(ancestor,siblings);
  }
  const roots=[...nodes];roots[entry.path[0]]=replacement;return roots;
}
