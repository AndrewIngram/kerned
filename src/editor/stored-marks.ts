...
export function inputMarks<N extends NodeIdentity>(schema:Schema<N>,state:EditorState<N>,tree=indexTree(schema,state.nodes)):readonly Mark[]{
  ...
  if(anchor.id===head.id){const node=tree.byId.get(anchor.id)?.node;

return node?marksAt(schema,node,Math.min(anchor.offset,head.offset),anchor.offset===head.offset?'left':'right'):[];}
