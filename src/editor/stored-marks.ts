...
export function marksAt<N extends NodeIdentity>(schema:Schema<N>,node:N,offset:number,side:'left'|'right'='left'):Mark[]{
  ...
  return adapter?.read(node).filter(range=>side==='right'?range.from<=offset&&range.to>offset:offset===0?range.from===0:range.from<offset&&range.to>=offset).map(range=>range.mark)??[];
}
