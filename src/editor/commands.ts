...
export function commandActivity(values:Iterable<boolean>):CommandActivity{
  ...
  for(const value of values){if(value)yes=true;else no=true;
...
if(!enabled)return chain;
 ...
 if (enabled && !(typeof command==='function'?command:command.execute)(chain, ...args)) enabled = false;
