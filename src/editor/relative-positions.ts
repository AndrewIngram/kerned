...
function string(value: unknown): string {if (typeof value !== 'string' || !value) throw new Error('Invalid position identity'); return value;}
function association(value: unknown): -1 | 1 {if (value !== -1 && value !== 1) throw new Error('Invalid association'); return value;}
...
export function createRelativePositions<N extends NodeIdentity>(schema: Schema<N>, initial: State<N>, documentId: string, checkpoint?: unknown) {
  ...
  function replayAfter(revision: number): ReturnType<typeof createMappingIndex> {
    const cached = suffixes.get(revision); if (cached) return cached;
    ...
    suffixes.set(revision, index); suffixWeight += index.weight; return index;
  }
  function changesAfter(revision: number) {
    ...
    if (!historyGroups) {
      ...
      for (const event of events) for (const op of event.operations) {
        let list = occurrences.get(op.id); if (!list) {list = []; occurrences.set(op.id, list);}
    ...
    const effects = historyGroups.flatMap(group => {
      ...
      while (low < high) {const mid = (low + high) >>> 1; if (group.revisions[mid] <= revision) low = mid + 1; else high = mid;}
  ...
  function resolvePoint(point: Point | null, bias: -1 | 1): RelativePositionResult {
    ...
    let stops = graphemes.get(node); if (!stops) {stops = boundaries(text); graphemes.set(node, stops);}
    let low = 0, high = stops.length;
    while (low < high) {const mid = (low + high) >>> 1; if (stops[mid] < point.offset) low = mid + 1; else high = mid;}
  ...
  function resolve(position: RelativePosition): RelativePositionResult {
    const error = check(position); if (error) return error;
  ...
  const api = Object.freeze({
    ...
    resolveRange(range: RelativeRange): RelativeRangeResult {
      const error = check(range.start) ?? check(range.end); if (error) return error;
      ...
      if (a.status !== 'resolved') return a; if (b.status !== 'resolved') return b;
      tree(); const first = textRanks.get(a.point.id), last = textRanks.get(b.point.id);
