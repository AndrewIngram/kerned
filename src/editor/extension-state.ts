...
export type ExtensionUpdate<N extends NodeIdentity>={before:EditorState<N>;after:EditorState<N>} & (
...
export function createStateField<N extends NodeIdentity,Value>(spec:{
  ...
  return Object.freeze({
    ...
    prepare(event:ExtensionUpdate<N>){values.set(event.after,{value:spec.update(read(event.before),event)});},
  });
}
