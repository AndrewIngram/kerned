...
export function FindBar({
  ...
  const [query, setQuery] = useState(initialQuery),
    [matchCase, setMatchCase] = useState(initialOptions.matchCase);

  const pending = stale || query !== state.query || matchCase !== state.matchCase;
  useLayoutEffect(() => {
    input.current?.focus({ preventScroll: true });
    input.current?.select();
    // oxlint-disable-next-line react/exhaustive-effect-dependencies -- A focus request explicitly refocuses and selects the existing input.
  }, [focusRequest]);
    ...
    >
      <input
        ...
        onChange={(event) => {
          ...
          onQuery(event.target.value, { matchCase });
        }}
      />
      <output
        id={statusId}
      ...
      <button
        ...
        onClick={() => {
          ...
          onQuery(query, { matchCase: !matchCase });
        }}
      >
        Aa
      </button>
