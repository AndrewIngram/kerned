...
export function observeEditorViewport(options:{
  element:HTMLElement;scrollport:HTMLElement|Window;toolbar?:HTMLElement|null;
  ...
  const measure=()=>onChange({width:element.clientWidth,inset:toolbar?.offsetHeight??0,
    height:Math.max(1,page?scrollport.innerHeight-(toolbar?.offsetHeight??0):scrollport.clientHeight),
    scrollTop:page?scrollport.scrollY:scrollport.scrollTop});
  const observer=new ResizeObserver(measure);observer.observe(element);if(toolbar)observer.observe(toolbar);
  ...
  measure();let destroyed=false;
  return ()=>{if(destroyed)return;destroyed=true;observer.disconnect();scrollport.removeEventListener('scroll',measure);if(page)scrollport.removeEventListener('resize',measure);};
}
