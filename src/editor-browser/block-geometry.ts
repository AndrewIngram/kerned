import type { DrawingRect } from './drawing';

/** Shared allocation for text, native blocks, measurements and node decorations. */
export function allocatedBlockWidth(width: number, inset: number) {
  return Math.max(80, width - inset);
}

/** Layer text coordinates include the inherited inset; node chrome starts after it. */
export function allocatedBlockBounds(block: DrawingRect & { readonly inset: number }): DrawingRect {
  return {
    left: block.left + block.inset,
    top: block.top,
    width: allocatedBlockWidth(block.width, block.inset),
    height: block.height,
  };
}
