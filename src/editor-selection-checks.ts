...
export function checkSelections(){
 ...
 check(!editor.state.selection.isEmpty(context())&&textSelection(1,0).isEmpty(context()),'Cursor and range emptiness');
 const json=JSON.parse(JSON.stringify(editor.selectionJSON()));check(editor.readSelection(json).eq(editor.state.selection),'Text codec round-trip');
 const edit=editor.selectionEdit('X');dispatch(edit.steps,edit.selection);equal(editor.state.nodes.map(node=>node.kind==='text'?node.text:node.kind),['AlXta','Gamma'],'Cross-block replacement removes atom and joins text');editor.undo();check(editor.state.selection instanceof TextSelection&&editor.state.selection.anchor.id===1&&editor.state.selection.head.id===2,'Undo restores cross-block bookmark');editor.redo();editor.undo();
 editor.select(textSelection(1,1,4));dispatch([{kind:'split',id:1,at:2,rightId:4,rightKey:'split'}]);check(editor.state.selection instanceof TextSelection&&editor.state.selection.anchor.id===1&&editor.state.selection.head.id===4,'Split preserves endpoints in separate nodes');editor.undo();
 editor.select(new NodeSelection(9));dispatch([{kind:'moveChildren',parent:null,index:1,count:1,toParent:null,toIndex:3}]);check(editor.state.selection.eq(new NodeSelection(9)),'Node selection follows move');editor.undo();
 const nodeJSON=editor.selectionJSON();check(editor.readSelection(nodeJSON).eq(new NodeSelection(9)),'Node codec');
 dispatch(editor.selectionEdit('').steps);check(editor.state.selection instanceof TextSelection,'Deleted node falls back to text');editor.undo();check(editor.state.selection.eq(new NodeSelection(9)),'Undo restores selected node');
 editor.select(new AllSelection());check(editor.readSelection(editor.selectionJSON()) instanceof AllSelection,'All codec');const clear=editor.selectionEdit('');dispatch(clear.steps,clear.selection);check(editor.state.nodes.length===0&&editor.state.selection instanceof AllSelection,'Empty document supports all selection');editor.undo();check(editor.state.nodes.length===4&&editor.state.selection instanceof AllSelection,'Whole-document undo');
 const atomOnly=createEditor(schema,[{id:7,key:'only',kind:'atom'}],new NodeSelection(7));const removal=atomOnly.selectionEdit('');atomOnly.dispatch({baseRevision:0,origin:'local',history:'separate',time:0,...removal});check(atomOnly.state.selection instanceof AllSelection,'Atom-only deletion needs no invented text node');
 ...
 equal(cellSelection.cells(cellContext()),[24,21],'Rectangle selects disjoint first column with head first');
 equal(cellSelection.content(cellContext()).fragments.map(fragment=>fragment.kind==='text'?fragment.text:''),['D','A'],'Cell content excludes unrelated cells');
 equal(cellSelection.ranges(cellContext()).map(range=>range.id),[34,31],'Cell ranges exclude structurally intervening cells');check(cells.readSelection(JSON.parse(JSON.stringify(cells.selectionJSON()))).eq(cellSelection),'Cell codec');
 const replacement=cells.selectionEdit('Z');cells.dispatch({baseRevision:0,origin:'local',history:'separate',time:0,...replacement});equal([31,32,33,34,35,36].map(id=>cellContext().text(id)),['','B','C','Z','E','F'],'Cell replacement inserts in head only and preserves unselected cells');cells.undo();check(cells.state.selection.eq(cellSelection),'Undo restores cell selection');cells.redo();cells.undo();
 const map=tables.grid(cellContext(),10);check(tables.grid(cellContext(),10)===map,'Grid cached by table identity');
 cells.dispatch({baseRevision:cells.state.revision,origin:'stream',history:'exclude',steps:[{kind:'append',nodes:[leaf(50,'stream')]}]});check(tables.grid(cellContext(),10)===map,'Unrelated streamed root does not invalidate grid');
 cells.select(new tables.CellSelection(10,21,24,'column'));equal(cells.state.selection.ranges(cellContext()).map(range=>range.id),[34,31],'Column extent');
 cells.dispatch({baseRevision:cells.state.revision,origin:'local',history:'separate',time:0,steps:[{kind:'insertChildren',parent:10,index:2,nodes:[container(13,'row',[container(27,'cell',[leaf(37,'G')]),container(28,'cell',[leaf(38,'H')]),container(29,'cell',[leaf(39,'I')])])]}]});equal(cells.state.selection.ranges(cellContext()).map(range=>range.id),[34,31,37],'Column selection includes inserted row');cells.undo();
 cells.select(cellSelection);cells.dispatch({baseRevision:cells.state.revision,origin:'local',history:'separate',time:0,steps:[{kind:'removeChildren',parent:null,index:0,count:1}]});check(cells.state.selection instanceof TextSelection,'Removed table selection falls back');cells.undo();check(cells.state.selection.eq(cellSelection),'Undo recovers table bookmark');
 const spanning=container(100,'table',[container(101,'row',[container(110,'cell',[leaf(120,'span')],2,2),container(111,'cell',[leaf(121,'top')])]),container(102,'row',[container(112,'cell',[leaf(122,'bottom')])])]);
