...
if(last?.font===chosen)last.end=end;else segments.push({start,end,font:chosen});
    }
  ...
  function prepare(text: string, spans: Span[], width: number, size: number, cached?: RetainedParagraph, requestedHeight=size*1.6, grid=0): PreparedParagraph {
    ...
    if (shaped) stats.cacheHits++;
      ...
      for (const glyph of glyphs) {
        ...
        if (!cluster || cluster.start !== glyph.start) { cluster = { start: glyph.start, end: text.length, width: 0, glyphs: [], stops: [] };

 if (clusters.length) clusters[clusters.length - 1].end = glyph.start; clusters.push(cluster); }
