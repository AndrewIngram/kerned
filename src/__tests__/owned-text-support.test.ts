import { expect, test } from 'vitest';

import { supportsOwnedText } from '../owned-text-support';

test('printable ASCII and newline agree with the composer control-character policy', () => {
  const printable = Array.from({ length: 95 }, (_, index) => String.fromCharCode(index + 32)).join(
    '',
  );

  expect(supportsOwnedText(printable + '\n')).toBe(true);

  const controls = [...Array.from({ length: 32 }, (_, index) => index), 127].filter(
    (code) => code !== 10,
  );

  for (const code of controls) {
    expect(supportsOwnedText('text' + String.fromCharCode(code))).toBe(false);
    expect(supportsOwnedText('élan' + String.fromCharCode(code))).toBe(false);
  }
});

test.each([
  ['Café e\u0301lan', true],
  ['“Typography”—yes!', true],
  ['👨‍👩‍👧‍👦 🇬🇧 1️⃣', true],
  ['a\u200db', false],
  ['a\u2028b', false],
  ['a\u2029b', false],
  ['a\u202eb', false],
  ['مرحبا', false],
])('retains the Unicode support policy for %s', (text, accepted) => {
  expect(supportsOwnedText(text)).toBe(accepted);
});
