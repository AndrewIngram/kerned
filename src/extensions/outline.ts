...
export type OutlineEntry=Readonly<NodeIdentity & OutlineHeading & {depth:number;parentKey:string|null}>;
 ...
 */
export function createOutlineExtension<N extends NodeIdentity>(schema:Schema<N>,heading:(node:N)=>OutlineHeading|null){
  const cache=new WeakMap<N,readonly (NodeIdentity & OutlineHeading)[]>();
  const empty:readonly OutlineEntry[]=[];
  let previous:readonly N[]|undefined,previousPending:readonly OutlineEntry[]|undefined,result:readonly OutlineEntry[]=[];
  function collect(node:N):readonly (NodeIdentity & OutlineHeading)[]{
    ...
    if(value&&(!Number.isSafeInteger(value.level)||value.level<1))throw new Error('Outline levels must be positive integers');
    const entries: (NodeIdentity & OutlineHeading)[]=value?[{id:node.id,key:node.key,level:value.level,title:value.title}]:[];
    for(const child of schema.children(node))entries.push(...collect(child));
    cache.set(node,entries);return entries;
  }
  return {
    read(nodes:readonly N[],pending:readonly OutlineEntry[]=empty):readonly OutlineEntry[]{
      ...
      const next:OutlineEntry[]=[],ancestors:OutlineEntry[]=[];
      ...
      for(const entry of [...headings,...pending]){
        ...
        const item={...entry,depth:ancestors.length,parentKey:ancestors.at(-1)?.key??null};
        next.push(item);ancestors.push(item);
      }
      previous=nodes;previousPending=pending;result=next;return result;
    },
  };
}
