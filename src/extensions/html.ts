...
export function importHtml(html:string):HtmlImport{
  ...
  function append(value:string,marks:Marks){
    ...
    if(marks.bold||marks.italic||marks.underline){
      ...
      if(last&&last.end===start&&last.bold===marks.bold&&last.italic===marks.italic&&!!last.underline===marks.underline)last.end=text.length;
      else spans.push({start,end:text.length,bold:marks.bold,italic:marks.italic,...(marks.underline?{underline:true}:{})});
    }
  }
...
if(stops[mid]<offset)lo=mid+1;else hi=mid;}
      ...
      nodes.push({...headingLevel?{kind:'heading',level:headingLevel}:{kind:'paragraph'},id,key:`html-${id}`,text,
        marks:formattingMarks(spans.filter(s=>s.start<text.length).map(s=>({...s,start:snap(s.start,false),end:snap(Math.min(s.end,text.length),true)}))),inline:[]});
    }
  ...
  function visit(node:Node,marks:Marks){
    if(node.nodeType===Node.TEXT_NODE){append(node.textContent??'',marks);
...
return;}
    ...
    if(['BLOCKQUOTE','UL','OL','LI'].includes(tag)){
      ...
      if(!children.length){const id=nextId++;children=[{kind:'paragraph',id,key:`html-${id}`,text:'',marks:[],inline:[]}];}

      if(tag==='UL'||tag==='OL')children=children.map(child=>{if(child.kind==='listItem')return child;const id=nextId++;
...
return {kind:'listItem',id,key:`html-${id}`,children:[child]};});
      ...
      nodes.push(tag==='BLOCKQUOTE'?{kind:'quote',...identity,children}:tag==='LI'?{kind:'listItem',...identity,children}:{kind:'list',...identity,ordered:tag==='OL',start:Number.isSafeInteger(start)&&start>0?start:1,children});
      tables+=imported.tables;

for(const key of ['headings','links','superscripts','subscripts','nestedTables'] as const)conversions[key]+=imported.conversions[key];
    ...
    if(node instanceof HTMLTableElement){
      ...
      const rows=[...node.rows].map((row,rowIndex)=>[...row.cells].map((cell):TableCell=>{
        ...
        const content=imported.nodes.flatMap((block):TextBlockNode[]=>block.kind==='paragraph'||block.kind==='heading'?[block]:block.kind==='table'?[{kind:'paragraph',id:0,key:'',text:tablePlainText(block),marks:[],inline:[]}]:[]);
        ...
        const paragraphs:TextBlockNode[]=content.length?content.map(p=>{const id=nextId++;

return {...p,id,key:`html-${id}`};}):[{kind:'paragraph',id:nextId,key:`html-${nextId++}`,text:'',marks:[],inline:[]}];

        const id=nextId++;

return {kind:'tableCell',id,key:`html-${id}`,row:rowIndex,header:cell.tagName==='TH',colspan:cell.colSpan,rowspan,paragraphs};
      }));
    ...
    if(tag==='BR'){text=text.replace(/ $/,'')+'\n';

return;}

    const block=blocks.has(tag),heading=/^H[1-6]$/.test(tag);

    if(block)flush();
    const previousHeading=headingLevel;

    if(heading){const level=Number(tag.slice(1));headingLevel=level===1?1:level===2?2:level===3?3:4;conversions.headings++;}
    ...
    const next={bold:marks.bold||tag==='B'||tag==='STRONG'||tag==='TH'||style?.fontWeight==='bold'||Number(style?.fontWeight)>=600,italic:marks.italic||tag==='I'||tag==='EM'||style?.fontStyle==='italic',underline:marks.underline||tag==='U'||!!style?.textDecoration.includes('underline')};
  ...
  for(const child of template.content.childNodes)visit(child,{bold:false,italic:false,underline:false});
  flush();
