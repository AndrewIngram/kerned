...
export function importHtml(html:string):HtmlImport{
  ...
  function flush(force=false){
    ...
    if(text||force){
      ...
      function snap(offset:number,up:boolean){
        ...
        while(lo<hi){const mid=(lo+hi)>>>1;if(stops[mid]<offset)lo=mid+1;else hi=mid;}
  ...
  function visit(node:Node,marks:Marks){
    ...
    if(['BLOCKQUOTE','UL','OL','LI'].includes(tag)){
      ...
      tables+=imported.tables;for(const key of ['headings','links','superscripts','subscripts','nestedTables'] as const)conversions[key]+=imported.conversions[key];
      return;
    }
    if(node instanceof HTMLTableElement){
      ...
      const rows=[...node.rows].map((row,rowIndex)=>[...row.cells].map((cell):TableCell=>{
        ...
        const paragraphs:TextBlockNode[]=content.length?content.map(p=>{const id=nextId++;return {...p,id,key:`html-${id}`};}):[{kind:'paragraph',id:nextId,key:`html-${nextId++}`,text:'',marks:[],inline:[]}];
