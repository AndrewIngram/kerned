...
if(!element)return;
  ...
  return <div ref={ref} tabIndex={-1} className="table-block" data-editor-interactive data-table={node.id} onKeyDown={e=>{
    if((e.metaKey||e.ctrlKey)&&['b','i','u'].includes(e.key.toLowerCase())){e.preventDefault();onFormat(e.key.toLowerCase()==='b'?'bold':e.key.toLowerCase()==='i'?'italic':'underline');

return;}

    if((e.metaKey||e.ctrlKey)&&e.key.toLowerCase()==='z'){e.preventDefault();onUndo(e.shiftKey);
      ...
      <tbody>{node.rows.map((row,rowIndex)=><tr key={rowIndex}>{row.map((cell,cellIndex)=>{
        ...
        return <Cell key={cell.id} colSpan={cell.colspan} rowSpan={cell.rowspan} data-cell={cell.id} data-selected={selected.has(cell.id)}>
          ...
          {cell.paragraphs.map((paragraph,i)=>editing===paragraph.id?<textarea data-text-block={paragraph.id} style={paragraph.kind==='heading'?{fontSize:typography(paragraph,18).size,lineHeight:`${typography(paragraph,18).lineHeight}px`,fontWeight:700}:undefined} ref={textRef} key={paragraph.id} aria-label={`Cell ${rowIndex+1}, ${cellIndex+1} text`} value={paragraph.text} onBlur={e=>{if(e.relatedTarget instanceof HTMLElement&&!ref.current?.contains(e.relatedTarget))setEditing(null);}} onSelect={e=>{
            const el=e.currentTarget;const next=textSelection(paragraph.id,el.selectionStart,el.selectionEnd);

if(!selection.eq(next))onSelect(next);
          }} onChange={e=>{
            const value=e.target.value,old=paragraph.text;let from=0;

while(from<old.length&&from<value.length&&old[from]===value[from])from++;
            let end=old.length,tail=value.length;

while(end>from&&tail>from&&old[end-1]===value[tail-1]){end--;tail--;}

            onText(paragraph.id,from,end,value.slice(from,tail),e.target.selectionStart);
          }} onKeyDown={e=>{if(e.key==='Tab'){e.preventDefault();const paragraphs=node.rows.flat().flatMap(c=>c.paragraphs),index=paragraphs.findIndex(p=>p.id===paragraph.id),next=paragraphs[index+(e.shiftKey?-1:1)];

if(next){setEditing(next.id);onSelect(textSelection(next.id,0));}}}}/>:<button data-text-block={paragraph.id} className="cell-content" key={paragraph.id} aria-label={`Edit cell ${rowIndex+1}, ${cellIndex+1}${i?`, paragraph ${i+1}`:''}`} onClick={e=>{if(e.shiftKey)selectCell(cell,true);else{setEditing(paragraph.id);onSelect(textSelection(paragraph.id,0));}}}><CellText paragraph={paragraph} matches={findMatches?.get(paragraph.id)} activeMatch={activeMatch}/>{!paragraph.text&&<span className="empty-cell">&nbsp;</span>}</button>)}
        </Cell>;
      })}</tr>)}</tbody>
    </table>
  </div>;
