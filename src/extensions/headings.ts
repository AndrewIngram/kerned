...
export function setTextBlockType(schema:Schema<HybridNode>,state:EditorState<HybridNode>,ids:readonly number[],level:HeadingLevel|null,tree=indexTree(schema,state.nodes)):Step<HybridNode>[] {
  return ids.flatMap(id=>{
    ...
    const content={id:node.id,key:node.key,text:node.text,spans:node.spans,atoms:node.atoms,comments:node.comments};
...
export function selectedBlockLabel(schema:Schema<HybridNode>,state:EditorState<HybridNode>,tree=indexTree(schema,state.nodes)){
  ...
  for(const range of ranges){
    if(!empty&&range===ranges.at(-1)&&range.kind==='text'&&range.from===0&&range.to===0)continue;
    let entry=tree.byId.get(range.id);if(!entry)continue;
    const node=entry.node;
    if(node.kind==='heading'){labels.add(`Heading ${node.level}`);continue;}
    let label=node.kind==='image'?'Image':node.kind==='checklist'?'Checklist':'Paragraph';
    while(entry){
      if(entry.node.kind==='table'||entry.node.kind==='tableCell'){label='Table';break;}
      if(entry.node.kind==='list'){label=entry.node.ordered?'Numbered list':'Bullet list';break;}
