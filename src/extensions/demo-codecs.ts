...
function integer(value:unknown,min=0){const result=jsonNumber(value);

if(!Number.isSafeInteger(result)||result<min)throw new Error('Expected an integer in range');

return result;}

function textNode(node:StarterNode):TextBlockNode{if(node.kind!=='paragraph'&&node.kind!=='heading')throw new Error('Expected text node');
...
function cell(node:StarterNode):TableCell{if(node.kind!=='tableCell')throw new Error('Expected table cell');

return node;}

function textCodec(kind:'paragraph'|'heading'):NodeCodec<StarterNode>{return {
  encode(node):JsonValue{const data=textData(node);
...
if(level!==1&&level!==2&&level!==3&&level!==4)throw new Error('Invalid heading level');
...
function atom(kind:'checklist'|'image'):NodeCodec<StarterNode>{return {
  ...
  decode(value,{identity}){const data=jsonRecord(value);

return kind==='checklist'?{kind,...identity,checked:jsonArray(data.checked).map(jsonBoolean),expanded:jsonBoolean(data.expanded),notes:jsonString(data.notes)}:{kind,...identity,src:jsonString(data.src),alt:jsonString(data.alt)};},
};}
...
function container(kind:'quote'|'listItem'|'list'):NodeCodec<StarterNode>{return {
  ...
  decode(value,{identity,children}){const data=jsonRecord(value);

return kind==='list'?{kind,...identity,children,ordered:jsonBoolean(data.ordered),start:integer(data.start,1)}:{kind,...identity,children};},
};}
...
export const demoCodecs:Readonly<Record<string,NodeCodec<StarterNode>>>={
  ...
  tableCell:{
    encode(node):JsonValue{const value=cell(node);

return {row:value.row,header:value.header,colspan:value.colspan,rowspan:value.rowspan};},
    decode(value,{identity,children}){const data=jsonRecord(value);

return {kind:'tableCell',...identity,row:integer(data.row),header:jsonBoolean(data.header),colspan:integer(data.colspan,1),rowspan:integer(data.rowspan,1),paragraphs:paragraphs(children)};},
  },
  table:{
    encode(node):JsonValue{if(node.kind!=='table')throw new Error('Expected table');
...
const rows=lengths.map((length,index)=>{const row=cells.slice(offset,offset+length);offset+=length;

if(row.some(cell=>cell.row!==index))throw new Error('Cell row index does not match table structure');
