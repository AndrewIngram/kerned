...
function attributes(value: JsonValue): MentionData {
  const data = jsonRecord(value),
    label = jsonString(data.label),
    width = jsonNumber(data.width),
    ascent = jsonNumber(data.ascent),
    descent = jsonNumber(data.descent);

  if (width < 0 || ascent < 0 || descent < 0) throw new Error('Invalid mention dimensions');

  return { label, width, ascent, descent };
}
...
export const mention: InlineValueExtension<InlineAtom> = {
  ...
  version: 1,
  parse: attributes,
  plainText: (value) => attributes(value).label,
