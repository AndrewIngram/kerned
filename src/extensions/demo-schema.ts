...
function textBlock(node:StarterNode):TextBlockNode{
  if((node.kind!=='paragraph'&&node.kind!=='heading'))throw new Error('Text extension requires a paragraph or heading');
...
export const paragraphExtension:NodeExtension<StarterNode>={
  ...
  editing:{
    ...
    marks:{validate:marks=>marks.map(mark=>formattingSchema.create(mark.type,mark.attrs)),boundary:formattingSchema.boundary,read:node=>textBlock(node).marks,write(node,marks){const text=textBlock(node);

return {...text,marks:formattingSchema.validate(text.text,marks)};}},
    replace(node,from,to,text){const next=replaceText(textBlock(node),from,to,text);validateInlineObjects(next.text,next.inline);

return next;},
    split(node,at,right){const p=textBlock(node);const tail={...slice(p,at,p.text.length,right.id),key:right.key};

return [slice(p,0,at),at===p.text.length?{kind:'paragraph',id:tail.id,key:tail.key,locked:tail.locked,text:tail.text,marks:tail.marks,inline:tail.inline}:tail];},
