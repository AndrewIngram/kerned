...
function textBlock(node: StarterNode): TextBlockNode {
  if (node.kind !== 'paragraph' && node.kind !== 'heading')
    throw new Error('Text extension requires a paragraph or heading');
...
function joined(left: TextBlockNode, right: TextBlockNode): TextBlockNode {
  ...
  return {
    ...
    marks: normalizeMarks([
      ...left.marks,
      ...right.marks.map((range) => ({
        ...range,
        from: range.from + offset,
        to: range.to + offset,
      })),
    ]),
    inline: [...left.inline, ...right.inline.map((a) => ({ ...a, index: a.index + offset }))],
  };
}
...
export const paragraphExtension: NodeExtension<StarterNode> = {
  ...
  version: 2,
  kind: 'text',
  accepts: (node) => node.kind === 'paragraph',
  validateUpdate(before, after) {
    const a = textBlock(before),
      b = textBlock(after);

    if (a.text !== b.text || a.inline !== b.inline)
  ...
  editing: {
    ...
    marks: {
      validate: (marks) => marks.map((mark) => formattingSchema.create(mark.type, mark.attrs)),
      boundary: formattingSchema.boundary,
      read: (node) => textBlock(node).marks,
      write(node, marks) {
        const text = textBlock(node);

        return { ...text, marks: formattingSchema.validate(text.text, marks) };
      },
    },
    replace(node, from, to, text) {
      const next = replaceText(textBlock(node), from, to, text);
      validateInlineObjects(next.text, next.inline);

      return next;
    },
    split(node, at, right) {
      const p = textBlock(node);
      const tail = { ...slice(p, at, p.text.length, right.id), key: right.key };
...
export const headingExtension: NodeExtension<StarterNode> = {
  ...paragraphExtension,
  name: 'heading',
  accepts: (node) => node.kind === 'heading',
};

export const checklistExtension: NodeExtension<StarterNode> = {
  name: 'checklist',
  version: 1,
  kind: 'atom',
  accepts: (node) => node.kind === 'checklist',
  validateUpdate() {},
};

export const imageExtension: NodeExtension<StarterNode> = {
  name: 'image',
  version: 1,
  kind: 'atom',
  accepts: (node) => node.kind === 'image',
  validateUpdate() {},
};

const codecsByName = new Map(Object.entries(demoCodecs));

export const demoStarterKit: NodeExtension<StarterNode>[] = [
  paragraphExtension,
  headingExtension,
  checklistExtension,
  imageExtension,
  editableTableExtension,
  tableCellExtension,
  quoteExtension,
  ...listCommands.extensions,
].map((extension) => ({ ...extension, codec: codecsByName.get(extension.name) }));
