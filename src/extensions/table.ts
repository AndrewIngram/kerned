...
for(const child of children){if(child.kind!=='tableCell')throw new Error('Expected cell');(rows[child.row]??=[]).push(child);}
...
for(const child of children){if((child.kind!=='paragraph'&&child.kind!=='heading'))throw new Error('Cells require paragraphs');paragraphs.push(child);}
