...
export function createTable(allocate:()=>NodeIdentity,rows=3,columns=3):TableNode{
 return {kind:'table',...allocate(),caption:'',rows:Array.from({length:rows},(_,row)=>Array.from({length:columns},()=>({kind:'tableCell',...allocate(),row,header:row===0,colspan:1,rowspan:1,paragraphs:[{kind:'paragraph',...allocate(),text:'',spans:[],atoms:[],comments:[]}]})))};
}
