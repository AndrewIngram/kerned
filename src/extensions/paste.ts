...
export function pasteParagraphs(schema:Schema<StarterNode>,state:EditorState<StarterNode>,text:string,allocate:()=>NodeIdentity){
  ...
  const middle:StarterNode[]=lines.slice(1,-1).map(text=>({kind:'paragraph',...allocate(),text,marks:[],inline:[]}));
  const steps:Step<StarterNode>[]=[...replacement.steps,
