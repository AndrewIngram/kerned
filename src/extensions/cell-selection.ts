  ...
  function buildGrid(rows:readonly (readonly GridCell[])[]){
    ...
    for(let y=0;y<height;y++){
      ...
      for(const cell of rows[y]){
        ...
        for(let row=y;row<y+cell.rowspan;row++)for(let col=x;col<x+cell.colspan;col++){const slot=row*width+col;

if(slots[slot]!==-1)throw new Error('Overlapping table cells');slots[slot]=index;}
...
for(const cell of result.cells)if(!context.node(cell.id))throw new Error('Missing grid cell');cache.set(node,result);
...
return result;
  ...
  function rectangle(context:SelectionContext,selection:CellSelection){
    ...
    if(selection.extent==='row'){rect.left=0;rect.right=map.width;}

if(selection.extent==='column'){rect.top=0;rect.bottom=map.height;}
  ...
  class CellSelection extends Selection{
    ...
    ranges(context:SelectionContext):SelectionRange[]{
      ...
      function visit(id:number){const text=context.text(id);

if(text!==null)ranges.push({kind:'text',id,from:0,to:text.length});else {const children=context.children(id);

if(children.length)children.forEach(node=>visit(node.id));else ranges.push({kind:'node',id});}}
...
return {type:this.type,version:1,data:{table:context.node(this.tableId)?.key,anchor:context.node(this.anchorCell)?.key,head:context.node(this.headCell)?.key,extent:this.extent}};}
    replace(context:SelectionContext,text:string):SelectionEdit{
      ...
      for(const range of [...ranges].sort((a,b)=>(rank.get(b.id)??0)-(rank.get(a.id)??0))){
        ...
        else {const location=context.location(range.id);
...
return point?textSelection(point.id,point.offset).getBookmark():{map(){return this;},resolve:context=>selectionNear(context)};
    }
    resolve(context:SelectionContext):Selection{const selection=new CellSelection(this.tableId,this.anchor,this.head,this.extent);

try{selection.validate(context);

return selection;}catch{return selectionNear(context);}}
  }
  ...
  const extension:SelectionExtension={type:'cell',read(context,value){
    ...
    const table=context.byKey(value.table),anchor=context.byKey(value.anchor),head=context.byKey(value.head);

if(!table||!anchor||!head)throw new Error('Missing cell selection keys');

return new CellSelection(table.id,anchor.id,head.id,value.extent);
  }};
