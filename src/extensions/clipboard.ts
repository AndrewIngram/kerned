...
export function pasteFragment(schema:Schema<StarterNode>,state:EditorState<StarterNode>,fragment:Fragment,allocate:()=>NodeIdentity){
 ...
 function covered(node:StarterNode):boolean{const r=selected.get(node.id);if(r?.kind==='node')return true;const text=schema.text(node);if(text!==null)return r?.kind==='text'&&r.from===0&&r.to===text.length;const children=schema.children(node);return children.length>0&&children.every(covered);}
 ...
 if(state.selection instanceof NodeSelection){const location=selectionContext(schema,state.nodes).location(state.selection.id);if(!location)throw new Error('Missing paste destination');return {steps:[{kind:'replaceChildren',...location,count:1,nodes:inserted} satisfies Step<StarterNode>],selection};}
