...
for(let i=0;i<edges.length-1;i++){const from=edges[i],to=edges[i+1],marks=node.marks.filter(s=>s.from<=from&&s.to>=to);let part=escape(plainText(node,from,to)).replaceAll('\n','<br>');

if(marks.some(s=>s.mark.type==='bold'))part=`<strong>${part}</strong>`;

if(marks.some(s=>s.mark.type==='italic'))part=`<em>${part}</em>`;

if(marks.some(s=>s.mark.type==='underline'))part=`<u>${part}</u>`;body+=part;}
...
return `<${tag}>${body}</${tag}>`;
  ...
  case 'list':{const tag=node.ordered?'ol':'ul';
...
return `<${tag} start="${node.start}">${node.children.map(html).join('')}</${tag}>`;}
  ...
  case 'tableCell':{const tag=node.header?'th':'td';
...
export function writeClipboard(data:DataTransfer,schema:Schema<StarterNode>,state:EditorState<StarterNode>,text:string){
...
return selected.length?[schema.withChildren(node,selected)]:[];
 ...
 const token=crypto.randomUUID();fragments.set(token,{nodes,inline});

if(fragments.size>8){const first=fragments.keys().next().value;
...
export function pasteFragment(schema:Schema<StarterNode>,state:EditorState<StarterNode>,fragment:Fragment,allocate:()=>NodeIdentity){
 if(fragment.nodes.length===1&&fragment.nodes[0].kind==='table'){const rectangle=pasteCellRectangle(schema,state,fragment.nodes[0],allocate);

if(rectangle)return rectangle;}

 function clone(node:StarterNode):StarterNode{
  const children=schema.children(node);
  const copy:StarterNode=node.kind==='paragraph'||node.kind==='heading'?{...node,...allocate(),inline:node.inline.map(a=>({...a,id:crypto.randomUUID()}))}:{...node,...allocate()};
 ...
 for(const {node} of all){const text=schema.text(node);

if(text!==null&&!supportsOwnedText(text))throw new Error('This study currently supports Latin text and emoji.');}

 const ranges=state.selection.ranges(selectionContext(schema,state.nodes)),selected=new Map(ranges.map(r=>[r.id,r]));

 function covered(node:StarterNode):boolean{const r=selected.get(node.id);

if(r?.kind==='node')return true;const text=schema.text(node);

if(text!==null)return r?.kind==='text'&&r.from===0&&r.to===text.length;const children=schema.children(node);
...
return children.length>0&&children.every(covered);}
 ...
 if(state.nodes.every(covered))return {steps:[{kind:'replaceChildren',parent:null,index:0,count:state.nodes.length,nodes:inserted} satisfies Step<StarterNode>],selection};

 if(state.selection instanceof NodeSelection){const location=selectionContext(schema,state.nodes).location(state.selection.id);

if(!location)throw new Error('Missing paste destination');

return {steps:[{kind:'replaceChildren',...location,count:1,nodes:inserted} satisfies Step<StarterNode>],selection};}
 ...
 if(state.selection instanceof RangeSelection&&!ranges.some(range=>range.kind==='text')){
  ...
  return {steps:[...removal.steps,{kind:'insertChildren',parent:location.parent,index,nodes:inserted} satisfies Step<StarterNode>],selection};
 }
...
if(!entry)throw new Error('Missing paste destination');
 ...
 const steps:Step<StarterNode>[]=[...removal.steps,{kind:'split',id:caret.id,at:caret.from,rightId:tail.id,rightKey:tail.key}];
