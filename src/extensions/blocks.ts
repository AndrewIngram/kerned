...
export const listCommands=createListExtensions<StarterNode>({
 ...
 withChildren(node,children){if(!('children'in node))throw new Error('Expected a container');
...
export const quoteExtension:NodeExtension<StarterNode>={
 ...
 content:{children:node=>'children'in node?node.children:[],withChildren(node,children){if(node.kind!=='quote')throw new Error('Expected quote');
...
export function projectBlocks(roots:StarterNode[]){
 ...
 function visit(node:StarterNode,inset:number,quotes:readonly {id:number;inset:number}[],marker=''){
  if(node.kind==='quote'){node.children.forEach(child=>visit(child,inset+24,[...quotes,{id:node.id,inset}]));

return;}

  if(node.kind==='list'){node.children.forEach((child,index)=>visit(child,inset+28,quotes,node.ordered?`${node.start+index}.`:'•'));

return;}

  if(node.kind==='listItem'){node.children.forEach((child,index)=>visit(child,inset,quotes,index===0?marker:''));

return;}

  if(node.kind==='tableCell')throw new Error('Table cells must belong to a table');
  nodes.push(node);decorations.set(node.id,{inset,quotes,marker});
 }
...
export function blockCommands(schema:Schema<StarterNode>,state:EditorState<StarterNode>,ids:number[],allocate:()=>NodeIdentity,tree=indexTree(schema,state.nodes)){
 ...
 function ancestor(kind:'quote'|'list'|'listItem'){
  ...
  while(entry){if(entry.node.kind===kind)return entry;entry=entry.parent===null?undefined:tree.byId.get(entry.parent);}
 }

 function siblings(){
  if(!first||entries.some(e=>e.parent!==first.parent))throw new Error('Select blocks in the same container');
  const indexes=entries.map(e=>e.index),index=Math.min(...indexes),last=Math.max(...indexes);
 ...
 return {
  ...
  quoted:entries.length>0&&entries.every(entry=>{while(entry){if(entry.node.kind==='quote')return true;const parent=entry.parent===null?undefined:tree.byId.get(entry.parent);

if(!parent)return false;entry=parent;}

return false;}),
  quote():Step<StarterNode>[]{
   if(!entries.length)return [];

   const ancestors=(id:number)=>{const result:number[]=[];let entry=tree.byId.get(id);

while(entry){result.push(entry.node.id);entry=entry.parent===null?undefined:tree.byId.get(entry.parent);}
...
return result;};
   ...
   if(existing&&paths.every(path=>path.includes(existing.node.id)))return [{kind:'unwrap',id:existing.node.id}];
   ...
   const parent=paths[0].slice(1).find(id=>paths.every(path=>path.slice(1).includes(id)))??null;
  ...
  list(ordered:boolean):Step<StarterNode>[]{
   ...
   if(existing?.node.kind==='list'){
    ...
    return [...existing.node.children.map(item=>({kind:'unwrap' as const,id:item.id})),{kind:'unwrap',id:existing.node.id}];
   }

   const range=siblings();

return listCommands.wrap(schema,state,range.parent,range.index,range.count,{ordered,start:1},allocate).steps;
  },
 };
}
...
export function replaceStructuredText(schema:Schema<StarterNode>,state:EditorState<StarterNode>,text:string){
 ...
 const start=ai<hi||ai===hi&&selection.anchor.offset<=selection.head.offset?selection.anchor:selection.head;
 const end=start===selection.anchor?selection.head:selection.anchor;
 const chosen=leaves.slice(Math.min(ai,hi),Math.max(ai,hi)+1),first=chosen[0];

 if((first?.kind!=='paragraph'&&first?.kind!=='heading'))throw new Error('Expected text start');

 const ranges:SelectionRange[]=chosen.map(node=>node.kind==='paragraph'||node.kind==='heading'
