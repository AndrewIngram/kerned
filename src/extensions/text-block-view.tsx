...
const DecorationView=createReactRenderers<CommentViewProps>([{name:'comment',component:({value})=>{
  ...
  const paint=useMemo<Painter>(()=>(canvas,k,brush)=>{brush.setColor(k.Color(246,234,180));

for(const {rect:r} of decorations)canvas.drawRect(k.XYWHRect(r[0],y+r[1],r[2]-r[0],r[3]-r[1]),brush);},[decorations,y]);
...
const MarkView=createReactRenderers<{id:number;y:number;underlines:{rect:number[];baseline:number}[]}>([{name:'underline',component:({value:{id,y,underlines}})=>{
  const paint=useMemo<Painter>(()=>(canvas,k,brush)=>{brush.setColor(k.Color(41,50,39));

for(const {rect:r,baseline} of underlines)canvas.drawRect(k.XYWHRect(r[0],y+baseline+2,r[2]-r[0],1),brush);},[underlines,y]);
