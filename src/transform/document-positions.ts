...
export type SnapshotTransition<N> = Readonly<{
  before: DocumentSnapshot<N>;
  after: DocumentSnapshot<N>;
  maps: readonly PositionMap[];
}>;

type Location =
  | { kind: 'text'; id: number; offset: number }
  | { kind: 'gap'; parent: number | null; index: number };
...
export type SnapshotRange = Readonly<{
  from: SnapshotPosition;
  to: SnapshotPosition;
  backward: boolean;
}>;

export type MappedSnapshotPosition =
  | { status: 'mapped'; position: SnapshotPosition }
  | { status: 'deleted' };
 ...
 */
export function createPositionSnapshot<N extends NodeIdentity>(
  schema: Schema<N>,
  state: { nodes: readonly N[]; revision: number },
): PositionSnapshot<N> {
  ...
  for (let i = tree.order.length - 1; i >= 0; i--) {
    const node = tree.order[i].node,
      type = schema.resolve(node);

    const size =
      type.kind === 'text'
        ? type.editing.text(node).length + 2
        : type.kind === 'atom'
          ? 1
  ...
  for (const { node } of tree.order) {
    if (schema.resolve(node).kind === 'container')
      indexChildren(node.id, schema.children(node), startOf(node.id) + 1);
  }
  ...
  function requirePosition(point: SnapshotPosition) {
    ...
    if (!value)
      throw new Error('Position belongs to a different snapshot; resolve or map it first');
  ...
  function gap(parent: number | null, index: number): SnapshotPosition {
    ...
    if (!Number.isSafeInteger(index) || index < 0 || index >= ranks.length)
      throw new Error('Invalid child gap');

    return make(
      { kind: 'gap', parent, index },
      ranks[index],
      parent === null ? null : entry(parent).node,
    );
  }

  function compare(a: SnapshotPosition, b: SnapshotPosition): -1 | 0 | 1 {
    const left = requirePosition(a).rank,
      right = requirePosition(b).rank;
  ...
  return Object.freeze({
    ...
    text(id: number, offset: number): SnapshotPosition {
      const node = entry(id).node,
        value = schema.text(node);

      if (value === null) throw new Error('Text position requires a text node');
      let stops = textStops.get(id);

      if (!stops) {
        stops = new Set(boundaries(value));
        textStops.set(id, stops);
      }

      if (!Number.isSafeInteger(offset) || !stops.has(offset))
        throw new Error('Text position must follow grapheme boundaries');
    ...
    before(id: number): SnapshotPosition {
      ...
      return gap(target.parent, target.index);
    },
    after(id: number): SnapshotPosition {
      const target = entry(id);

      return gap(target.parent, target.index + 1);
    },
    ...
    mapTo(
      target: PositionSnapshot<N>,
      point: SnapshotPosition,
      transition: SnapshotTransition<N>,
      bias: -1 | 1 = 1,
    ): MappedSnapshotPosition {
      requirePosition(point);

      if (
        target.revision !== state.revision + 1 ||
        transition.before.nodes !== state.nodes ||
        transition.before.revision !== state.revision ||
        transition.after.nodes !== target[snapshotState].nodes ||
        transition.after.revision !== target.revision
      )
        throw new Error('Mapping does not connect these snapshots');
    ...
    [mappedLocation](location: Location, bias: -1 | 1): MappedSnapshotPosition {
      ...
      const node = tree.byId.get(location.id)?.node,
        value = node ? schema.text(node) : null;
      ...
      const offset =
        bias === 1
          ? stops.find((at) => at >= location.offset)
          : stops.toReversed().find((at) => at <= location.offset);
      ...
      return {
        status: 'mapped',
        position: make(
          { kind: 'text', id: location.id, offset },
          startOf(location.id) + 1 + offset,
          node,
        ),
      };
    },
    range(anchor: SnapshotPosition, head: SnapshotPosition): SnapshotRange {
      ...
      return Object.freeze({
        from: backward ? head : anchor,
        to: backward ? anchor : head,
        backward,
      });
    },
    commonAncestor(a: SnapshotPosition, b: SnapshotPosition): N | null {
      const left = ancestors(requirePosition(a).node),
        right = ancestors(requirePosition(b).node);

      let common: N | null = null;

      for (let i = 0; i < Math.min(left.length, right.length) && left[i].id === right[i].id; i++)
        common = left[i];
