...
export function createEditorScene(owned:Owned,size=20) {
  ...
  function firstAt(y:number){
    ...
    while(lo<hi){const mid=(lo+hi)>>>1,p=previous.placements[mid];

if(p.y+p.height<y)lo=mid+1;else hi=mid;}
  ...
  return {
    build(nodes:StarterLeaf[],width:number,measurements:ReadonlyMap<number,Measurement>,view:View,decorations:ReadonlyMap<number,BlockDecoration>=new Map()) {
      ...
      if(reflow){dirty.clear();

for(const [id,value] of cache)if(value.width!==Math.max(80,width-(decorations.get(id)?.inset??0)))dirty.set(id,value.node);}
...
return !value||value.layout!==null;
      ...
      function update(node:TextBlockNode){const started=performance.now(),value=compose(node,Math.max(80,width-(decorations.get(node.id)?.inset??0)));compositionMs+=performance.now()-started;cache.set(node.id,value);dirty.delete(node.id);layoutIds.push(node.id);
...
function offsetLayout(layout:LaidOut,inset:number):LaidOut{
 ...
 hit:(x,y)=>layout.hit(x-inset,y),geometry:(a,h,u)=>{const g=layout.geometry(a,h,u);
