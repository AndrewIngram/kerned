...
export function createEditorScene(owned:Owned,size=20) {
  ...
  function firstAt(y:number){
    let lo=0,hi=previous.placements.length;
    while(lo<hi){const mid=(lo+hi)>>>1,p=previous.placements[mid];if(p.y+p.height<y)lo=mid+1;else hi=mid;}
    return lo;
  }
  return {
    build(nodes:StarterLeaf[],width:number,measurements:ReadonlyMap<number,Measurement>,view:View,decorations:ReadonlyMap<number,BlockDecoration>=new Map()) {
      ...
      if(reflow){dirty.clear();for(const [id,value] of cache)if(value.width!==Math.max(80,width-(decorations.get(id)?.inset??0)))dirty.set(id,value.node);}
      ...
      const viewportReady=previous.placements.slice(firstAt(view.top-160),firstAt(view.top+view.height+160)+1)
        .every(p=>(p.node.kind!=='paragraph'&&p.node.kind!=='heading')||p.layout!==null)&&view.pinned.every(id=>{
          const value=cache.get(id);return !value||value.layout!==null;
        });
      if(nodes===previousNodes&&width===previous.width&&measurements===previousMeasurements&&!paddingChanged&&!dirty.size&&viewportReady){
        previous={...previous,top:view.top,zoom:view.zoom};
        return {scene:previous,layoutIds,reflow,background:0,compositionMs,workMs:performance.now()-started};
      }
      if(nodes!==previousNodes){
        const live=new Set(nodes.map(n=>n.id));
        for(const id of cache.keys())if(!live.has(id)){cache.delete(id);dirty.delete(id);owned.release(id);}
      }
      const oldTop=view.top*view.zoom/previous.zoom;
      const oldAnchorIndex=firstAt(oldTop),anchor=previous.placements[oldAnchorIndex];
      const retainedAnchor=anchor?nodes.findIndex(n=>n.id===anchor.node.id):-1;
      const anchorIndex=retainedAnchor>=0?retainedAnchor:Math.min(oldAnchorIndex,nodes.length-1);
      const offset=anchor?oldTop-anchor.y:oldTop-32;
      const first=firstAt(oldTop-160),pinned=new Set(view.pinned);
      let anchorY=anchor?.y??32+padding,y=32+padding,newLayouts=0;
      const placements:Placement[]=[];
      function update(node:TextBlockNode){const started=performance.now(),value=compose(node,Math.max(80,width-(decorations.get(node.id)?.inset??0)));compositionMs+=performance.now()-started;cache.set(node.id,value);dirty.delete(node.id);layoutIds.push(node.id);return value;}
      ...
      for(let index=0;index<nodes.length;index++){
        const node=nodes[index],inset=decorations.get(node.id)?.inset??0,layoutWidth=Math.max(80,width-inset);
        if(index>0){const prev=nodes[index-1];const after=prev.kind==='paragraph'||prev.kind==='heading'?typography(prev,size).after:24;const current=nodes[index];const before=current.kind==='paragraph'||current.kind==='heading'?typography(current,size).before:0;y+=Math.max(after,before);}
        if(index===anchorIndex)anchorY=y;
        if((node.kind!=='paragraph'&&node.kind!=='heading')){
          const measured=measurements.get(node.id);
          const height=measured?.width===width?measured.height:node.kind==='image'?96:node.kind==='table'?Math.max(60,node.rows.length*64):node.expanded?290:190;
        ...
        if(!value&&(urgent||newLayouts<128)||value&&urgent&&(changed||value.width!==layoutWidth||!value.layout)){
          if(!value)newLayouts++;
          value=update(node);
        }
        else if(!value){
          const style=typography(node,size),columns=Math.max(1,Math.floor(layoutWidth/(style.size*.5)));
          ...
          cache.set(node.id,value);dirty.set(node.id,node);
        }
          ...
          if(changed&&value.layout){value={...value,layout:null,boxes:[]};cache.set(node.id,value);owned.releaseLayout(node.id);}
        }
        else dirty.delete(node.id);
        if(!value)throw new Error('Missing paragraph layout');
        placements.push({node,y,height:value.height,layout:value.layout?offsetLayout(value.layout,inset):null,layoutWidth:width,boxes:value.boxes.map(box=>({...box,x:box.x+inset}))});y+=value.height;
      }
      let background=0;
      ...
      if(view.advance&&!view.eager){
        ...
        for(const [id,node] of dirty){
          if(cache.has(id)){update(node);background++;}
      ...
      if(background){
        ...
        for(let index=0;index<placements.length;index++){
          const p=placements[index],value=(p.node.kind==='paragraph'||p.node.kind==='heading')?cache.get(p.node.id):undefined;
          if(index>0){const prev=nodes[index-1];const after=prev.kind==='paragraph'||prev.kind==='heading'?typography(prev,size).after:24;const current=nodes[index];const before=current.kind==='paragraph'||current.kind==='heading'?typography(current,size).before:0;y+=Math.max(after,before);}
        if(index===anchorIndex)anchorY=y;
          const inset=decorations.get(p.node.id)?.inset??0;
          const next=value?{...p,y,height:value.height,layout:value.layout?offsetLayout(value.layout,inset):null,layoutWidth:width,boxes:value.boxes.map(box=>({...box,x:box.x+inset}))}:{...p,y};
          placements[index]=next;y+=Math.ceil(next.height/4)*4;
        }
      }
      ...
      const top=anchor&&!keepStart?anchorY+Math.min(offset,newAnchor?.height??offset):view.top;
      ...
      if(!view.retainAll){
        ...
        for(let index=0;index<placements.length;index++){
          ...
          if((p.node.kind!=='paragraph'&&p.node.kind!=='heading')||pinned.has(p.node.id)||index>=Math.max(0,first-48)&&index<end+48)continue;
          const value=cache.get(p.node.id);
          if(value?.layout){cache.set(p.node.id,{...value,layout:null,boxes:[]});owned.releaseLayout(p.node.id);}
    ...
    get residentParagraphs(){return [...cache.values()].filter(p=>p.layout).length;},
  };
}
...
function offsetLayout(layout:LaidOut,inset:number):LaidOut{
 ...
 return {...layout,draw:(canvas,x,y)=>layout.draw(canvas,x+inset,y),
 drawViewport:layout.drawViewport?(canvas,x,y,top,bottom)=>layout.drawViewport!(canvas,x+inset,y,top,bottom):undefined,
 hit:(x,y)=>layout.hit(x-inset,y),geometry:(a,h,u)=>{const g=layout.geometry(a,h,u);return {caret:shift(g.caret),rects:g.rects.map(shift)};}};
}
