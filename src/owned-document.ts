...
export function placeParagraphs(paragraphs: ComposedParagraph[]) {
  ...
  let overflowAbove = 0, overflowBelow = 0;
  ...
  const lines = placements.flatMap(p => p.paragraph.lines.map(line => ({
    ...line, start: line.start + p.offset, end: line.end + p.offset,
    top: line.top + p.y, bottom: line.bottom + p.y, baseline: line.baseline + p.y,
  })));
  ...
  function translated(rect: Rect, y: number): Rect { return [rect[0], rect[1] + y, rect[2], rect[3] + y]; }
  ...
  function geometry(anchor: number, focus: number, upstream: boolean): Geometry {
    ...
    const caret = translated(p.paragraph.geometry(focus - p.offset, focus - p.offset, upstream).caret, p.y);
    const rects: Rect[] = [];

    if (anchor !== focus) {
      const low = Math.min(anchor, focus), high = Math.max(anchor, focus);
      const first = preceding(placements, low, 'offset'), last = preceding(placements, high, 'offset');

      for (let i = first; i <= last; i++) {
        const p = placements[i];
        const start = Math.max(0, low - p.offset), end = Math.min(p.paragraph.textLength, high - p.offset);

        for (const rect of p.paragraph.geometry(start, end, false).rects) rects.push(translated(rect, p.y));
      }
    }
  ...
  return {
    placements, height, lines, hit, geometry,
    *visible(top: number, bottom: number) {
      ...
      for (let i = preceding(placements, top - overflowBelow, 'y'); i < placements.length; i++) {
        ...
        if (placement.y + placement.paragraph.inkBottom > top && placement.y + placement.paragraph.inkTop < bottom) yield placement;
      }
    },
    move(index: number, upstream: boolean, direction: Direction): Position {
      const ordinal = preceding(placements, index, 'offset'), p = placements[ordinal];
      ...
      if (direction === 'up' || direction === 'down') {
        ...
        return hit(caret[0], (caret[1] + caret[3]) / 2 + (direction === 'up' ? -lineHeight : lineHeight));
      }
      ...
      if (direction === 'right' && local === p.paragraph.textLength && ordinal + 1 < placements.length) {
