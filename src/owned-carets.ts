...
export function createPackedCarets(
  capacity: number,
  lineCapacity: number,
  lines: Line[],
  lineHeight: number,
  ...
  function closest(x: number, line: number) {
    ...
    const candidate =
      next === end
        ? end - 1
        : next > first && Math.abs(xs[next - 1] - x) <= Math.abs(xs[next] - x)
          ? next - 1
          : next;
  ...
  return {
    storage() {
      ...
      return {
        caretBufferBytes: buffer.byteLength,
        caretUsedBytes: used,
        caretUnusedBytes: buffer.byteLength - used,
        caretCapacity: capacity,
        caretCount: count,
        lineCapacity,
        lineCount: rowCount,
      };
    ...
    append(index: number, x: number, upstream: boolean) {
      offsets[count] = index;
      xs[count] = x;
      affinities[count] = Number(upstream);
      count++;
    },
    finish() {
      rowStarts[rowCount] = count;
      ...
      capacity = count;
      lineCapacity = rowCount;
    },
    hit(this: void, x: number, y: number): Position {
      return position(closest(x, Math.floor(y / lineHeight)));
    },
    geometry(this: void, anchor: number, focus: number, upstream: boolean): Geometry {
      const ordinal = locate(focus, upstream),
        line = lines[rowOf(ordinal)];
      ...
      if (anchor !== focus) {
        ...
        for (let row = 0; row < rowCount; row++) {
          ...
          if (end - first > 1)
            rects.push([xs[first], lines[row].top, xs[end - 1], lines[row].bottom]);
        }
      }
    ...
    move(this: void, index: number, upstream: boolean, direction: Direction): Position {
      ...
      if (direction === 'home' || direction === 'end')
        return position(closest(direction === 'home' ? 0 : lines[row].width, row));
