...
if(!value)throw new Error(message);}
  ...
  for(const at of boundaries(paragraph.text)){
    ...
    const left=resolveAnchor(demoSchema,parseAnchor(JSON.parse(JSON.stringify(beforeAnchor))),'doc',editor.state,editor.journal);
    const right=resolveAnchor(demoSchema,afterAnchor,'doc',editor.state,editor.journal);
    check(left.status==='resolved'&&left.anchor.blockKey==='original'&&left.anchor.offset===at,'Split left affinity');
    check(right.status==='resolved'&&right.anchor.blockKey==='right'&&right.anchor.offset===0,'Split right affinity');
    ...
    editor.undo();equal(editor.state.nodes,result.state.nodes,'Undo join');editor.undo();equal(editor.state.nodes,[paragraph],'Undo split');
    editor.redo();editor.redo();equal(editor.state.nodes,[paragraph],'Redo split and join');
