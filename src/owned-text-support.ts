...
export function supportsOwnedText(text:string){
 ...
 return stops.slice(0,-1).every((start,i)=>{const value=text.slice(start,stops[i+1]);

return !/\p{Cf}/u.test(value)||emojiSequence.test(value)&&!/[\p{Cf}]/u.test(value.replaceAll('\u200d',''));});
}
