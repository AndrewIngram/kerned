...
export function supportsOwnedText(text:string){
 if(/[^\p{Script=Latin}\p{Script=Common}\p{Script=Inherited}]/u.test(text)||/[\p{Zl}\p{Zp}\u0000-\u0009\u000b-\u001f\u007f]/u.test(text))return false;
 if(!/\p{Cf}/u.test(text))return true;
 const stops=boundaries(text);
 return stops.slice(0,-1).every((start,i)=>{const value=text.slice(start,stops[i+1]);return !/\p{Cf}/u.test(value)||emojiSequence.test(value)&&!/[\p{Cf}]/u.test(value.replaceAll('\u200d',''));});
}
